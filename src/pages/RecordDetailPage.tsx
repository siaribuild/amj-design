  ...
  return (
    <section id={id} className="bg-white border border-black/10">
      <div className="flex items-center gap-2.5 px-5 py-[15px] border-b border-black/10 flex-wrap">
        ...
        {right && <span className="sm:ml-auto text-xs text-[#5c5a56] basis-full sm:basis-auto" style={{ fontFamily: "'DM Mono', monospace" }}>{right}</span>}
      </div>
      {children}
    </section>
  );
  ...
  return (
    <details className="border bg-white mt-3.5" style={{ borderColor: TONE.attn.bd }}>
      <summary className="list-none cursor-pointer px-4 py-3 flex items-center gap-2.5 text-sm font-medium text-[#131311] [&::-webkit-details-marker]:hidden">
        <Check className="w-4 h-4 text-[#5A7A6A]" />{summary}
      ...
      <div className="px-4 pb-4 border-t border-black/[0.07]">
        {receipt && (
          <div className="flex flex-col gap-2 py-3.5">
            {receipt.map((r) => (
              <div key={r.label} className={`flex justify-between text-[13.5px] text-[#5c5a56] ${r.big ? "border-t border-black/10 pt-2.5 mt-0.5 text-[15px]" : ""}`}>
                <span>{r.label}</span>
                <span className={r.big ? "font-medium" : "text-[#131311] font-medium"} style={{ fontFamily: "'DM Mono', monospace", fontVariantNumeric: "tabular-nums", color: r.big ? TONE.attn.text : undefined }}>{r.value}</span>
              </div>
            ))}
          </div>
        )}
        {lines.map((l) => (
          <div key={l} className="flex items-start gap-[9px] text-[12.5px] text-[#5c5a56] py-2 leading-normal">
  ...
  return (
    <section className="p-5 border" style={{ borderColor: TONE.attn.bd, borderLeft: `3px solid ${TONE.attn.text}`, background: `linear-gradient(180deg, ${TONE.attn.bg}, rgba(178,110,15,.03))` }}>
      <div className="flex items-center gap-2 mb-2.5 flex-wrap">
        ...
        {step && <span className="text-[11.5px] text-[#5c5a56]" style={{ fontFamily: "'DM Mono', monospace" }}>{step}</span>}
      </div>
      <h2 className="text-lg font-semibold text-[#131311] mb-1.5" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{title}</h2>
      {children}
    </section>
  );
}
  ...
  return (
    <div className="bg-white border border-black/10 p-4 mt-3.5">
      ...
      <div className="text-[13px] text-[#5c5a56] space-y-1" style={{ fontFamily: "'DM Mono', monospace" }}>
        ...
        <p>Reference <span className="text-[#131311] font-medium">{orderNo}</span></p>
      </div>
      <p className="text-xs text-[#5c5a56] mt-2.5">We confirm receipt by email and this page updates the moment the payment lands — nothing else to do here.</p>
    </div>
  );
}
    ...
    <div className="relative grid grid-cols-[32px_1fr] gap-[15px] pb-[22px] last:pb-1">
      ...
      <div className="pt-[3px] min-w-0">
        <div className="flex items-center gap-2.5 flex-wrap">
          <h4 className={`text-[15px] font-semibold ${n.state === "locked" ? "text-[#9a988f]" : "text-[#131311]"}`} style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{n.title}</h4>
          {n.pill && (
        ...
        <div className="text-[11.5px] text-[#5c5a56] mt-[3px]" style={{ fontFamily: "'DM Mono', monospace" }}>{n.status}</div>
...
const num = (t: ReactNode) => <span className="text-[#131311]">{t}</span>;
...
export function LineList({ lines, footerLabel, total, statusPill, showUnit }: {
  ...
  return (
    <>
      ...
      <div className="md:hidden px-[18px] py-[11px] bg-[#5A7A6A]/[0.07] border-b border-black/10 text-[10.5px] tracking-[0.08em] uppercase text-[#5c5a56]" style={{ fontFamily: "'DM Mono', monospace" }}>
        {lines.length} line{lines.length === 1 ? "" : "s"} · anchored by schedule code
      </div>
      {lines.map((l, idx) => (
        <div key={`${l.code}-${idx}`} className="grid grid-cols-[1fr_auto] md:grid-cols-[118px_1fr_110px_44px_104px_116px] gap-x-3.5 gap-y-2 px-5 py-3.5 border-b border-black/[0.07] last:border-b-0 hover:bg-[#5A7A6A]/[0.05] items-center">
          <div className="flex flex-col gap-1">
            <span className="text-sm font-medium text-[#131311] tracking-[0.02em]" style={{ fontFamily: "'DM Mono', monospace" }}>{l.code}</span>
            {l.room && <span className="text-[11px] text-[#5c5a56] bg-black/[0.045] px-1.5 py-px w-fit">{l.room}</span>}
          </div>
          <div className="md:order-none order-3 col-span-2 md:col-span-1">
            <span className="text-[13.5px] text-[#131311]">{l.productName}</span>
            {l.optionsSummary && <span className="block text-[11.5px] text-[#5c5a56] mt-0.5" style={{ fontFamily: "'DM Mono', monospace" }}>{l.optionsSummary}</span>}
          </div>
...
export function FilesBlock({ files, note }: { files: ApiFile[]; note?: string }) {
  return (
    <Blk eyebrow="Files" title="Documents & uploads" right="Grouped & revision-aware" id="rec-files">
      ...
      {files.map((f) => (
        <div key={f.id} className="flex items-center gap-3.5 px-5 py-[13px] border-b border-black/[0.07] last:border-b-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[11.5px] text-[#5c5a56] mt-0.5 uppercase" style={{ fontFamily: "'DM Mono', monospace" }}>{f.kind} · {(f.size / 1024).toFixed(0)} KB</div>
          </div>
      ...
      {note && <p className="px-5 pb-4 pt-2 text-[11px] text-[#5c5a56]" style={{ fontFamily: "'DM Mono', monospace" }}>{note}</p>}
    </Blk>
  );
}
...
function PayRow({ label, amount, state }: {
  ...
  return (
    <div className="flex justify-between items-center py-[9px] border-b border-black/[0.07] text-[13.5px] gap-2 flex-wrap">
      <span className="flex items-center gap-2 text-[#5c5a56]">{label}<StatusPill tone={state.tone}>{state.label}</StatusPill></span>
      <span className="font-medium text-[#131311]" style={{ fontFamily: "'DM Mono', monospace", fontVariantNumeric: "tabular-nums" }}>{money(amount)}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex-1 basis-[250px] bg-white border border-black/10 p-[18px]">
      ...
      <div className="mt-3.5 flex gap-2">
        ...
        <a href="tel:0390000000" className="inline-flex items-center gap-2 px-4 py-2 text-xs text-[#5c5a56] hover:text-[#131311]"><Phone className="w-3.5 h-3.5" />Call</a>
      </div>
    </div>
  );
}
      ...
      <div className="flex flex-col gap-[26px]">
        ...
        {order.stage === "deposit_invoiced" && (
          <ActionGate pill="Action needed from you" step="Step 4 of 9" title="Pay the 50% deposit to begin">
            <p className="text-[13.5px] text-[#5c5a56] max-w-[56ch]">Your order is created and the deposit invoice is issued. <b className="text-[#131311]">Shop drawings begin once the deposit lands</b> — the remaining 50% is due before despatch, after quality photos.</p>
        ...
        {order.stage === "drawings_shared" && (
          <ActionGate pill="Action needed from you" step="Step 5 of 9" title="Review & sign off your shop drawings">
            <p className="text-[13.5px] text-[#5c5a56] max-w-[56ch]">We've prepared detailed drawings for all <b className="text-[#131311]">{order.lineCount ?? lines.length} items</b> — every profile, dimension and hardware position. Check them against your site. <b className="text-[#131311]">Manufacturing only begins once you sign off</b>, so this is the moment to catch changes. Requesting a change is free at this stage.</p>
        ...
        {order.stage === "balance_invoiced" && (
          <ActionGate pill="Action needed from you" step="Step 8 of 9" title="Pay the final 50% balance to release despatch">
            <p className="text-[13.5px] text-[#5c5a56] max-w-[56ch]">Quality photos are shared (see Files). The balance of <b className="text-[#131311]">{money(bal?.amount)}</b> is due before we book delivery.</p>
        ...
        {order.stage === "balance_paid" && (
          <ActionGate pill="Action needed from you" step="Step 8 of 9" title="Confirm you're ready for despatch">
            <p className="text-[13.5px] text-[#5c5a56] max-w-[56ch]">Balance received and quality photos shared. Confirm everything looks right and we book your delivery (~2 weeks).</p>
            <ConfirmDetails
        ...
        {!["deposit_invoiced", "drawings_shared", "balance_invoiced", "balance_paid"].includes(order.stage) && (
          <section className="p-5 border border-black/10 bg-white flex items-center gap-3" style={{ borderLeft: `3px solid ${TONE.pos.text}` }}>
            ...
            <div>
              ...
              <p className="text-[12.5px] text-[#5c5a56]">{m.next}. We'll email you the moment a step needs your OK.</p>
            </div>
          </section>
        )}
