  ...
  return (
    ...
    <aside className="fixed inset-y-0 right-0 z-50 w-full md:w-[480px] md:max-w-[92vw] bg-white flex flex-col border-l border-black/10"
      role="dialog" aria-modal="true" aria-label={`Review pricing change — ${family}`}>
        ...
        {/* THE SCROLL BODY ENDS HERE and the footer is pinned below it — the
            branch's whole point: a rate-card Save that scrolled out of reach
            was a Save nobody could press on a long family.

            The tripwire and the note input that sat here on that branch are
            NOT carried. `bfe4e7ab` deleted them from main on evidence — "the
            rate-card save button refused the most ordinary edit there is",
            because `pct()` returns 100 whenever the old value was absent, so
            the >20% guard armed on every first price. Re-adding them with the
            footer would have reinstated a bug main had already measured and
            removed. */}
        </div>
