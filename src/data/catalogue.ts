...
export interface Category {
   ...
   *  sanity.config.ts). Ops-managed; absent sorts last, then by name. The
   *  built-in fallback carries it too, so a browser that never reaches
   *  Sanity still gets Windows before Doors. */
  orderRank?: string;
...
export interface Product {
   ...
   *  product listing sorts by now. Absent sorts last, then by name — the old
   *  hand-typed featuredOrder that used to sit here as a fallback is gone
   *  (every live product and the built-in fallback catalogue carry a rank). */
  orderRank?: string;
  seo?: SeoMeta;
}
...
// live product and the built-in fallback catalogue carry a rank — so the
// only fallback left is name, same as the category comparator.
...
export interface PostCategory {
  ...
  /** Drag-and-drop rank, set in Studio (see sanity.config.ts) — what the
   *  postCategories query sorts by now. Absent sorts last. */
  orderRank?: string;
