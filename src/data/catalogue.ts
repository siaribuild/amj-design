...
export interface Category {
  ...
  slug: string;
  name: string;
  /** Drag-and-drop rank wherever the catalogue is grouped, set in Studio (see
   *  sanity.config.ts). Ops-managed; absent sorts last. The built-in fallback
   *  carries it too, so a browser that never reaches Sanity still gets Windows
   *  before Doors. */
  orderRank?: string;
  /** DEPRECATED — the old hand-typed order field, superseded by orderRank.
   *  Kept only as a sort fallback for a category that predates the migration. */
  order?: number | null;
...
export interface Family {
  ...
  categorySlug: string;
  name: string;
  /** Drag-and-drop rank, set in Studio (see sanity.config.ts) — what the
   *  families query sorts by now. */
  orderRank?: string;
  /** The canonical operation every product in this family performs (awning,
   *  sliding, hinged…). The single source of truth for family → operation:
...
export interface Product {
  ...
  /** Drag-and-drop rank, set in Studio (see sanity.config.ts) — what every
   *  product listing sorts by now. */
  orderRank?: string;
  /** DEPRECATED — the old hand-typed order field, superseded by orderRank.
   *  Kept only as a sort fallback for a product that predates the migration. */
  featuredOrder: number;
  seo?: SeoMeta;
}

// Which option group shows first in the item builder (src/data/configurator.ts
// reads this) — ops-managed in Studio's "Option Types" list once ranked there.
// This built-in fallback preserves today's order (the retired hardcoded
// TYPE_ORDER constant) for a browser that never reaches Sanity, or before the
// live optionType documents have been ranked.
export let optionTypeOrder: Record<string, string> = {
  colour: "a000", hardware: "a001", flyscreen: "a002", installation: "a003",
};
...
const productDefinitions: Product[] = [
  {
    ...
    standardGlass: "5+8A+5mm Double Tempered Clear Glass",
  ...
  },
  {
    ...
    standardGlass: "5+12A+5mm Double Tempered Clear Glass",
  ...
  },
  {
    ...
    standardGlass: "6mm Low-e+15Ar+6mm Tempered Clear Glass",
  ...
  },
  {
    ...
    standardGlass: "6mm Low-e+15Ar+ómm Tempered Clear Glass",
  ...
  },
  {
    ...
    standardGlass: "6mm Low-e+22Ar+ómm Tempered Clear Glass",
    minWidth: 600, minHeight: 800, maxWidth: 900, maxHeight: 2200,
    notes: "",
    heroImage: "https://images.unsplash.com/photo-1699259160970-a42f68d2eb2e?w=1200&h=900&fit=crop&auto=format",
    gallery: ["https://images.unsplash.com/photo-1699259160970-a42f68d2eb2e?w=1200&h=900&fit=crop&auto=format", "https://images.unsplash.com/photo-1614595737476-42487331b8a1?w=1200&h=900&fit=crop&auto=format", "https://images.unsplash.com/photo-1774199616762-31d947dc7d35?w=1200&h=900&fit=crop&auto=format"],
  ...
  },
  {
    ...
    standardGlass: "5mm Low-e+9Ar+5mm Tempered Clear Glass",
  ...
  },
  {
    ...
    standardGlass: "6mm Low-e+22Ar+6mm Tempered Clear Glass",
    minWidth: 600, minHeight: 1500, maxWidth: 1300, maxHeight: 3500,
    notes: "",
    heroImage: "https://images.unsplash.com/photo-1580687104004-8e9b3d462526?w=1200&h=900&fit=crop&auto=format",
    gallery: ["https://images.unsplash.com/photo-1580687104004-8e9b3d462526?w=1200&h=900&fit=crop&auto=format", "https://images.unsplash.com/photo-1743510935745-b0cd869db5e8?w=1600&h=1000&fit=crop&auto=format", "https://images.unsplash.com/photo-1614595737476-42487331b8a1?w=1200&h=900&fit=crop&auto=format"],
  ...
  },
  {
    ...
    standardGlass: "5+12A+5mm DoubleTempered Clear Glass",
  ...
  },
  {
    ...
    standardGlass: "6mm Low-e+25Ar+6mm Tempered Clear Glass",
  ...
  },
  {
    ...
    standardGlass: "6mm Low-e+25Argon+6mm Tempered Clear Glass",
...
// orderRank is the drag-and-drop rank (see sanity.config.ts); a product still
// carrying only the old hand-typed featuredOrder (never dragged since the
// migration) falls back to that.
const byOrderRank = (a: Product, b: Product) => {
...
export const getProductsByCategory = (categorySlug: string, q?: ProductQuery): Product[] =>
  visible(products.filter(p => p.categorySlug === categorySlug), q).sort(byOrderRank);
export const getProductsByFamily = (familySlug: string, q?: ProductQuery): Product[] =>
  visible(products.filter(p => p.familySlug === familySlug), q).sort(byOrderRank);
...
export interface CatalogueData {
  ...
  posts?: Post[];
  /** Option type slug -> drag-and-drop rank, set in Studio ("Option Types" list,
   *  see sanity.config.ts). Decides which option group (Colour, Hardware,
   *  Flyscreen…) shows first in the item builder — see optionGroupsFor in
   *  src/data/configurator.ts. */
  optionTypeOrder?: Record<string, string>;
}
export function hydrateCatalogue(data: CatalogueData): void {
  ...
  // Object.keys(...).length, not a truthy check — {} is truthy in JS, and an
  // empty map here (nothing ranked yet) must NOT silently blank out the
  // built-in fallback order below.
  if (data.optionTypeOrder && Object.keys(data.optionTypeOrder).length) optionTypeOrder = data.optionTypeOrder;
...
export interface PostCategory {
  ...
  /** Drag-and-drop rank, set in Studio (see sanity.config.ts) — what the
   *  postCategories query sorts by now. */
  orderRank?: string;
  /** DEPRECATED — the old hand-typed order field, superseded by orderRank. */
  order?: number;
