...
// in the client.
//
// RETARGETED when navigation replaced the holding screen. The scaffold's
// heading was the literal word "ops2" and its facts panel printed the detected
// router base; both moved rather than disappeared, and their REASONS moved with
// them. The heading is now the destination the console lands on, and the base
// is printed on the Settings root (src/ops2/pages/DestinationRoot.tsx) — still
// "so a wrong one is visible rather than merely wrong", and now on a page an
// operator can be asked to read out over the phone.
...
test("the ops2 shell mounts at /ops2, with no sign-in of its own", async ({ page }) => {
  ...
  // `/` lands on Attention: the console opens on what needs a person, because
  // the governing constraint is that a delayed glance costs a working day.
...
test("a deep link under /ops2 renders the console, not a 404", async ({ page }) => {
  ...
  const response = await page.goto(`${OPS2}/settings`);
  expect(response?.status()).toBe(200);
  await expect(page.getByRole("heading", { name: "Settings", level: 1 })).toBeVisible();
  await expect(page.getByText("/settings", { exact: true })).toBeVisible();

  // And an address no destination claims still lands on something. The
  // scaffold's catch-all route said why: "a deep link that survives Cloudflare
  // Access lands on something rather than a blank page." An unmatched route
  // inside an IonRouterOutlet renders NOTHING, which behind Access is
  // indistinguishable from an outage.
  const stale = await page.goto(`${OPS2}/record/p_demo`);
...
test("no rendered corner exceeds the owner's 5px cap, in either platform mode", async ({ page }) => {
  ...
  //
  // Now measured with the NAVIGATION on screen, which is where the Ionic
  // components with the widest per-platform radii live: ion-popover is 10px on
  // iOS, ion-button 14px. Each is capped by name in src/ops2/theme/ionic.css,
  // and this is the backstop for forgetting the next one.
  //
