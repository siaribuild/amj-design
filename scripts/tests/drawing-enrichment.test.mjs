...
test("full-document turn contract batches renders and records and rejects vocabulary escape", () => {
  ...
  const repairSkill = makeFullDocumentAgentSkill(["W1"], [1, 2], ["repair_w1"]);
  assert.deepEqual(
    repairSkill.responseSchema.properties.records.items.properties.evidenceRenderId.enum,
    ["repair_w1"],
  );
  assert.equal(validateFullDocumentTurn(raw, ["W1"], [1, 2], ["repair_w1"]).records.length, 0);
...
test("full-document repair turns bind pending openings to active tagged crops without duplicate or final renders", async () => {
  ...
  const record = (tag, operations, unitRatios, evidenceRenderId, frameBoxPt, roomLabel) => ({
    ...
    basis: [`${tag} composition is visible on the current evidence image.`], note: null,
  });
  const result = await runFullDocumentAgent({
    fileId: "f1", scheduleRows,
    inspected: {
      ...
      pages: [{ pageNo: 1, text: "ELEVATION A W15 W1 D1", words: [] }],
      timings: { inventoryMs: 1, textMs: 1, wordsMs: 1, totalMs: 3 },
    },
    deps: {
      runTurn: async (input) => {
        inputs.push(input);
        if (input.turn === 1) return {
          memory: "Render Elevation A.", renderRequests: [{ pageNo: 1, dpi: 110 }],
          records: [], declines: [], complete: false,
        };
        if (input.turn === 2) return {
          memory: "W15 is legible; W1 and D1 need close-ups.", renderRequests: [], declines: [], complete: false,
          records: [
            record("W15", ["fixed"], [1], "fd_t001_01", [500, 100, 700, 300], "Ensuite"),
        ...
        assert.deepEqual(input.pendingTags, ["W1", "D1"]);
  ...
  const result = await runFullDocumentAgent({
    ...
    deps: {
      runTurn: async (input) => {
        inputs.push(input);
        if (input.turn === 1) return {
          memory: "Render the full elevation.", renderRequests: [{ pageNo: 1, dpi: 110 }],
          records: [], declines: [], complete: false,
        };
        if (input.turn === 2) return {
          memory: "All 13 openings need close-ups.", renderRequests: [], declines: [], complete: false,
          records: scheduleRows.map((row, index) => record(row.tag, "fd_t001_01", frame(index))),
        };
        if (input.turn === 3) {
          assert.equal(input.renderCatalog.length, 12);
          assert.deepEqual(input.renderCatalog.map((item) => item.repairTag), scheduleRows.slice(0, 12).map((row) => row.tag));
          return {
            memory: "The first 12 tagged close-ups are resolved.", renderRequests: [], declines: [], complete: false,
            records: scheduleRows.slice(0, 12).map((row, index) =>
              record(row.tag, `fd_t002_${String(index + 2).padStart(2, "0")}`, frame(index))),
          };
        }
        assert.deepEqual(input.pendingTags, ["W13"]);
        assert.deepEqual(input.renderCatalog.map((item) => item.repairTag), ["W13"]);
        return {
          memory: "The queued thirteenth close-up is resolved.", renderRequests: [], declines: [], complete: true,
          records: [record("W13", "fd_t003_14", frame(12))],
        };
      },
      render: async (request) => {
        renderCalls++;
