...
- **Batch turns; don't narrate.** Every turn that changed code pays a stop-gate toll (the Codex review when enabled, plus the security-guidance LLM diff review). Ending a turn to report progress nobody asked for pays that toll for nothing. **The orchestrator decides the batch** — it ends a turn only where the user must decide, approve, or see something (a decision gate, the mock gate, a finished-and-verifiable slice, an impasse, or a genuine need for input), not to announce "now starting step 3". When in doubt, keep working: the user can always interrupt, but a toll already paid cannot be refunded.
