...
**Manufacturer price**:
The figure AMJ quotes for a whole line (quantity never multiplies it) — a composite parent included: once set, the parent's total stops following Σ(segments), which keeps being computed and stored as the preliminary machinery and the fallback — stored ex-GST on the line beside the uplift percentage that produced the line total. Commercially sensitive cost data, staff-only: never on a customer surface, never readable by a Manufacturer partner account, never in log output — audit entries record the resulting line total only, and never the uplift beside it (total plus uplift derives the cost). Clearing restores the computed price — the rate card's figure on a simple line, the **current** Σ(segments) on a composite parent (NULL while any segment stays unpriced) — and it is an endpoint capability with no console control (owner D1, 2026-09-04): a wrong entry is corrected by entering the right price. Same mutable window as the typed override — before the quote is issued. Its arithmetic (basis conversion first, then uplift, cents rounding at each step, no $10 rounding) has one home: `src/data/manufacturerPrice.ts`.
...
**Drawing enrichment**:
The pass that goes back into the plans, per opening the schedule already found, for what the schedule cannot carry — accurate split, orientation and elevation. It never re-derives the schedule and never contradicts it: the schedule owns the size, the drawing owns the division. Inferred room names may support internal document matching but never populate a quote line; the schedule's own comment becomes that line's Note. Invisible to the customer, who sees only openings, dimensions and the product selected; its value is a better-chosen product, and *better accuracy* means *higher estimate accuracy*. Off is a supported state — and the default: the enrichment runs only when `AI_EXTRACTION_MODE` is `auto_drawings`; under `auto` the schedule table and the default split carry the quote exactly as before.
...
**Candidate outcome**:
The structured facts the estimator emits per candidate — tier, rank, per-axis deviation, price delta against the pick, exclusion constraints. Facts only, never sentences: each surface composes its own wording. This is the contract the ops derivation surface reads. Since R10's reversal (owner, 2026-09-02) the price delta is rendered on the ops "Why this product?" ladder -- the one money figure on that surface; exclusions and the learned layer remain unsurfaced.
...
The console's gate and landing surface (`HOME_PATH`, `src/ops2/attention/`): one group per destination that has work waiting, counts inside, every count a link to the area that owns it. It carries no action that changes data; zero is drawn as absence, and a degraded count is drawn as failure — never as zero. The four project counts are not read off the summary body — each derives from the queue's own rows through `selectProjects(rows, attentionQuery(key)).length` (`src/ops2/projects/queue.ts`), the same function the Projects page filters with, so a count and its list can never disagree. Distinct from the record's Attention filter below, which scopes one record's line list.
_Avoid_: dashboard, home screen, metrics page

**Attention prefilter**:
The Projects queue's fourth query axis (`src/ops2/projects/queue.ts`, alongside chip/refinements/search), set only by arriving from an Attention destination link (`?attn=<key>`) and consumed once — stripped from the address and cleared on any plain re-entry to the page. Not a funnel refinement: a refinement rides the current chip, which cannot represent "awaiting payment" under the default `Needs us` chip (ADR 0018). No control toggles it manually; the owner's default-NO ruling on ops controls (`ops-accumulates-unrequested-features`) applies. Distinct from the record's Attention filter below, which is a permanent pill scoping one record's own line list rather than a one-shot queue narrowing.
_Avoid_: refinement, attention chip, attention tab
