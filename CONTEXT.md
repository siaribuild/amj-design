...
The console's gate and landing surface (`HOME_PATH`, `src/ops2/attention/`): one group per destination that has work waiting, counts inside, every count a link to the area that owns it. It carries no action that changes data; zero is drawn as absence, and a degraded count is drawn as failure — never as zero. The four project counts are not read off the summary body — each derives from the queue's own rows through `selectProjects(rows, arrivalQuery(key)).length` (`src/ops2/projects/queue.ts`), the same function the Projects page filters with, so a count and its list can never disagree. Distinct from the record's Attention filter below, which scopes one record's line list. The AI-parse monitoring section sits below the groups — the page is the gate first, monitoring second.
_Avoid_: dashboard, home screen, metrics page

**Attention arrival**:
`?attn=<key>` on a Projects link from the Attention destination — a one-shot arrival instruction, never a query axis (ADR 0020, superseding 0018/0019: there is no attention axis; the only filter axis is `refinements`). On load it is validated against the closed key set (`submissions` / `inReview` / `readyToIssue` / `awaitingPayment`, unknown values stripped and ignored), applies the whole query — chip `all`, exactly the one funnel refinement the key names (`readyToIssue` → the existing `ready`), search cleared — and strips itself from the address, so a card of N opens exactly N rows. Ended by leaving the route: navigating off Projects resets the queue to `EMPTY_QUERY`. The four former prefilters are ordinary funnel refinements — six in one list (New submissions, Being priced, Ready to issue, Awaiting payment, Unresolved lines, In production), one counting rule (a control counts what it would leave, grill P6), so a panel selection composes with whatever chip, refinements and search are already on; under the default `Needs us` chip a filter like "Awaiting payment" legitimately reads a labelled `0` (owner ruling D1: leave the chip alone — the visible zero is what makes that safe). Attention's card counts use the same `arrivalQuery` through the same `selectProjects` (`src/ops2/projects/queue.ts`), so a card and its filter agree with nothing else on. Distinct from the record’s Attention filter below, which is a permanent pill scoping one record’s own line list.
_Avoid_: attention prefilter (the deleted fourth axis), refinement group, attention chip, attention tab
