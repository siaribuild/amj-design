...
2. Check the diff against the spec's acceptance criteria one by one using the `mattpocock-skills:code-review` skill's two axes: does the code follow this repo's standards, and does it do what the spec asked — including what the spec asked for that is *absent* from the diff.
