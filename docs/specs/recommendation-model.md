...
| A21 | On a **tie**, the learned layer prefers the **cheaper** of the equally-chosen products; equal or unavailable prices fall to a **stable** tiebreak (the sorted slug), never a coin flip | §4.8, AC-31 | **Owner ruling at acceptance, reversing AD30.** A unique modal product is unaffected. The tie cannot be resolved inside `aggregateShadow`, which holds slugs and counts and no prices — a price is per (product × size × options) and does not exist until an opening is being priced — so the model reports `leaders` and the **outcome builder** breaks the tie, where the candidates and their prices already are. Price chooses among the TIED ONLY: letting it reach past them into products with fewer observations would make the learned layer a second cheapest-wins rule with less evidence behind it. The stable key is the slug, which is also the ladder's final tiebreak (AC-5), so there is one convention rather than two. The owner's first instinct was a coin flip and he took the stable version: everything here is built so a decision stays explicable, and a random tiebreak would let the layer name different products for identical evidence on two reads. **Still dark** — the shadow reaches only the builder, and that is now asserted structurally rather than stated |
