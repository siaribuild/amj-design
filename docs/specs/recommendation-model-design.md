...
| AD18 | ~~Unpriceable-sorts-last applies **before** the tier-C deviation rule~~ — **REVERSED by the owner at acceptance (spec A19).** Deviation is compared first; unpriceable-sorts-last now applies only among candidates of equal deviation | The original divergence read A6's "sorts last within its tier" as categorical and the conformance review agreed. The owner ruled the other way: design §4.2's numbered sequence stands, because a reviewer's losing-candidate list is more useful led by "this is the best thermal answer and we cannot price it at this size" than by a priced worse one. Safe because selection reads `competing`, not sort position — and between two priceable candidates the priceability step is a no-op, so the order of candidates that can win is unchanged | `worker/lib/estimator/ladder.ts` `compareCandidates`; test `recommendation-ladder.test.mjs` "A18 within a tier, DEVIATION is compared before priceability" | **reversed** — owner ruling supersedes the adjudication |
...
| AD24 | ~~A split candidate that does not fit is `lastResort: true` (tier E), not excluded~~ — **REVERSED by the owner at acceptance (spec A20).** A split that does not fit is **excluded**, and persisted with a `dimensions` exclusion naming the breached axes | The original reading treated a non-fitting make-up as a reviewer-selectable near-answer (ops2 AC-3). The owner ruled otherwise: **ops can build their own splits**, so proposing a make-up that cannot physically be built is not help, it is noise among candidates a reviewer might actually pick. It is still *persisted* with its reason, so the list distinguishes "a split was considered and rejected on fit" from "no split was tried". **Not to be unified with AD20**, which stays: that is `selectForComposite`'s enumeration, where make-ups are the only candidates and excluding one would empty the run | `worker/lib/estimator/select.ts` `splitLadderCandidate` (`lastResort: false`) and the paired precondition on `lastResortIds`; tests `estimator-split.test.mjs` "A20 a split that cannot be built is EXCLUDED" and "A20 SAFETY" | **reversed** — owner ruling supersedes |
...
| AD30 | ~~A tie is not a preference: `preferredSlug` is null when the bucket has no unique modal product, so `wouldPrefer` is false for every candidate~~ — **REVERSED by the owner at acceptance (spec A21).** A tie now prefers the **cheaper** of the equally-chosen products, falling to a **stable** tiebreak when prices are equal or unavailable | The original objection — that a tie promoted by row order flips on insertion order and asserts a lead the evidence does not hold — is answered rather than overruled: the resolution is price, then the sorted slug, and neither depends on row order. The seam moved because it had to: `aggregateShadow` holds slugs and counts and no prices, since a price is per (product × size × options) and does not exist until an opening is being priced. So the model reports `leaders` (every product at maximum support) and the **outcome builder** breaks the tie, where the candidates and their prices already are. Price is a tiebreak among the tied ONLY — never a re-ranking that reaches products with fewer observations. The stable key is the slug, matching the ladder's own final tiebreak (AC-5); the owner's first instinct was a coin flip and he took the stable version, because a random tiebreak would let the layer name different products for identical evidence on two reads | `worker/lib/estimator/learning.ts` (`leadingSlugs`, `ShadowLookup.leaders`), `worker/lib/estimator/outcome.ts` (`resolvePreference`); tests `estimator-learning.test.mjs` "A21 …" ×3 and `recommendation-contract.test.mjs` "AC-32 the shadow model has no channel into the ladder, structurally" | **reversed** — owner ruling supersedes |
