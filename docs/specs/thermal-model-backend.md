...
### 11.1 Implementation-stage assumptions (developer, pipeline stage 4)

Judgement calls made while building, on top of the spec's A2–A16 and the design's
AD-T1–AD-T17. Each names where it is visible in the code so it can be checked rather than
taken on trust, and each is vetoable at acceptance like any other entry above.

| # | Assumption | Reasoning / where it is visible |
|---|---|---|
| **A17** | The orientation→SHGC mapping follows the **table both documents write down** — NE/NW ⇒ target 0.4, cap 0.5 — not the behaviour the shipped code accidentally had (NE/NW ⇒ 0.5, no cap, because `o.startsWith("N")` was tested before the branch that explicitly enumerates them) | **ADJUDICATED — the call stands.** The architect ruled for the documented table (design AD-T17) and the tester reached the same conclusion independently, on the better argument: the identical prefix unsoundness would have swallowed SE/SW, and the author guarded *those* by ordering the enumerated branch first, so NE/NW is the same guard misordered. Moves no estimate — the path had never executed in production (0 of 444 default rows carried an SHGC target). `SHGC_BY_ORIENTATION`, `worker/lib/estimator/thermal/computedBand.ts` |
| **A18** | `ThermalModelInputs.climateZone` is `number \| null`, where design §2.1 declares `number` | `resolveDefaultEnvelope` is typed `EnvelopeArchetype \| null`, so a non-null guarantee would need an assertion at the assembly seam. Null takes the same branch as CZ6 — it reads the dial — and TB-21 asserts that branch directly. `thermal/contract.ts`, `pipeline.thermalInputsFor` |
| **A19** | `contractVersion` rides on `ComputedBandResult` as well as inside the persisted derivation | So the pipeline copies the version the calculation used rather than re-importing the constant and risking two answers. `thermal/contract.ts`, asserted in `thermal-selection.test.mjs` |
| **A20** | `SOURCED_FIELDS` is published by the contract, and `inputsMissing` is derived from it uniformly | One list, so a field added to the contract cannot be silently omitted from the missing count (AD-T5's rule, mechanised). `thermal/contract.ts` |
| **A21** | TB-13 is pinned at `applyDefaultEnvelope` — a `human_override` requirement is never recomputed, whatever it contains — rather than at the `opening_instance` upsert | The `edited_fields` guard in the upsert is pre-existing code this feature does not touch, and testing it needs the DB path. **Noted by the tester as structurally vacuous today**: nothing in `worker/` yet assigns `human_override`, so the guard is correct, defensive and unreachable until a human-edit path exists. `pipeline.applyDefaultEnvelope`, `ai-pipeline.test.mjs` |
| **A22** | Three pure seams are exported for testability — `requirementSnapshot`, `modelReachCounters`, `tallyTiers` | TB-7, TB-9 and TB-37 would otherwise need a full `runAiExtraction` fake to observe. Each is the exact thing that lands in the column or the summary, so the test reads the artefact rather than inferring it from the SQL around it. `pipeline.ts`, `estimator/estimate.ts` |
| **A23** | The archetype and dial snapshots key on the derivation's **presence**, not on a basis name | Equivalent to design §2.5's "widen to any computed basis", and it cannot drift if a basis is ever added. `pipeline.requirementSnapshot` |
| **A24** | The profile GROQ does not project `_id`, and the persisted dial snapshot is a **projection** that omits `set_by` and `observations` | Identity never fetched cannot leak into a cross-account aggregate through a later careless mapping; a per-opening row on a customer's project has no business carrying a staff Access email. Raised by the security review; TB-15's four required fields are unaffected. `thermal/calibration.ts`, `pipeline.requirementSnapshot` |
| **A25** | A Sanity failure during a calibration read surfaces as a 500 rather than being reported as an empty catalogue | Reporting zero deliverable rows because the CMS was down would understate axis 3 to the person about to turn the dial — wrong in the expensive direction. An honest failure is better than a confident wrong number. `readCalibration`, `worker/routes/ops-thermal.ts` |
| **A26** | Every suite's dial fixture deliberately carries a value that is **not** today's default | TB-18 is a property of the suites, so it is given something to bite on: a test that had quietly pinned the business default would fail rather than pass by coincidence. `thermal-selection.test.mjs` (2.5), `ai-pipeline.test.mjs` (2.5), `thermal-default-band.test.mjs` (4.4) |
