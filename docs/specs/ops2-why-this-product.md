...
**Date:** 2026-08-25 · **Stage:** pipeline stage 1 (product-manager) · **Revision 25**
...
`9285d642`. **Phase 3a passed independent verification** — all sixteen criteria against
executed evidence; the tester traced §7.0's property against every derivation writer and
could not construct a path where one asserts an absence it did not establish. **The class is
a genuine property, not a convenient list.**

**Revision 25 folds in three tester observations, none of which blocks acceptance.**

1. **SNAP-AC-1 said "that product+variant's" figures, and that is not always what is
   captured.** On a customer glazing-only change the pick moves, the figures re-resolve with
   **no variant named** — the glass drives — while the row deliberately **retains its old
   `selected_variant_id`** (`projects.ts:579`). The row therefore stores a variant that
   disagrees with its figures. **This is ruled, not a defect** (design §4.3; SNAP-AC-14's
   intent is to record what the customer *chose*), but the criterion's wording described the
   wrong thing. Now corrected — and **the consequence for Phase 3b copy is written into
   WHY-AC-4**, because a panel that says *"this variant performs at X"* beside a stored
   variant id meaning something else is a copy problem waiting to happen. **§14 puts the
   substance to the owner at acceptance.** This is **§12 note 13's third instance**.
2. **X-AC-13 is new** — the split path's body whitelist is executed as a real attempt rather
   than trusted. Safe today by inspection (`ops.ts:953-966` maps an explicit whitelist, so
   `configurationSnapshot` and `selectedVariantId` are never read from the body); **a
   whitelist is a list layer, and list layers here have been wrong repeatedly.**
3. **A Phase 3b copy hazard is recorded where 3b's copy criteria live** (WHY-AC-9):
   present-and-null can mean *"no selection was made"* as well as *"no figure exists for this
   product"* — W7 leaves `product_slug` standing while nulling the variant. Not a 3a defect;
   a known case rather than a discovered one.

**Revision 24 scoped a criterion that was too strong.** SNAP-AC-16 caught the defect it was
written for **and then over-caught** two correct writers. **A6 (design §1.6): two classes of
writer, one honesty invariant** — *no writer may assert an absence it did not establish*.
Derivation writers are named **as a class with a stated property**, not a list of sites (§7.0).

**Revision 23 repaired a contradiction between two of this spec's own criteria** (SNAP-AC-1 vs
SNAP-AC-9 — A4, design §1.4). **Revision 22 reconciled the register against the design** after
two entries were reported open while the architect had already ruled both. **Revision 21
retracted a false urgency** the owner corrected. **Revision 20** recorded the assertion-that-
cannot-fail rule; **revision 19** corrected VIEW-AC-15; **revision 18** resolved VIEW-AC-1's
internal contradiction; **revision 17** carried the owner's veto of §13.17; **revision 16**
carried D11 and D12.
...
thermally-constrained line**, while the other 19 pass untouched. **And the catalogue importers
write the flag back on every run**, so removing it from the documents without removing it from
them buys days, not a fix (§6, CERT-AC-12).

And a third gap, now the widest of the three: **a product's thermal performance is not
recorded on the line that uses it.** The save paths store the product, the options and the
price; the Uw and SHGC that make the choice reviewable are looked up at selection time and
dropped. Under R3/D3 nothing may look them up at display time to recover them — so if a save
does not record them, they are gone.

## 2. Scope

### In scope

1. **The read surface.** A three-line "Why this product" panel on the line detail view, and
   the detail **screen** it leads to — its own route (D8) — carrying the requirement and its
   origin, the chosen product against it, why it won, and the 3–5 next-best alternatives by
   ladder rank. Composites included (R14–R17). A **thinner panel** for lines nobody's
   estimator ever evaluated (D6). **No action anywhere on it** (R28).
...
3. **The universal performance-figure capture (R22, D3, D6, D7).** Every save that **moves a
   line's pick** — and every **validated re-derivation** by a derivation writer (§7.0) —
   records the resolved configuration's Uw and SHGC on the line, ops console and customer site
...
4. **A shared drawing viewer** for ops2 (R21, R25) — **a node in the navigation tree with its
   own route and a back control** (R31), **and reachable from the project record's desk
   canvas** (D11, §8.1).
5. **Two changes to the shared `SidePanel`** (R26, R29): full-screen on the phone, and a
   **back** control in place of "Done" — without moving the Projects filter panel.
...
| **Opportunistic backfill of pre-capture lines on touch** | Same ruling. A lookup at edit time is a display-time catalogue read wearing a snapshot's clothes (SNAP-AC-10's reasoning), and it blurs the three states. |
| **Conditioning the aiManaged branch on a new "material change" predicate** | A6's rejected alternative. The branch re-derives price, both snapshots and figures together; splitting figures out would describe one variant at two different times in adjacent columns. The residual is named in §7.0. |
...
- **Phase 3a — a server write-path change (§7)** · **verified, awaiting acceptance.**
  `worker/**`, so **Probity applies**. It carries the feature's only regression risk.
...
to any test of the display surface.
...
>
> **That is false, and it is retracted here rather than deleted.** The owner: **there will be
> no saves between 3a and 3b. The phasing exists to make the change manageable, nothing more.**
...
**R25 — no explanatory notation on an ops drawing surface.** **A class, not a block.** What
goes: anything teaching the reader how to read the drawing. What stays: statements about its
**authority**.
...
the 3a and 3b deployments*** · **D6** a client/manual-picked line shows a thinner panel **with**
its product's figures · **D7** the capture extends to the customer save path · **D8** the detail
...
- **A5 (§1.5) — a singleton answer set is not ambiguity.** Ambiguity, and its null, begin at
  **two** surviving variants. **Vetoable at acceptance as one pair with the design's ambiguity
  rule.**
- **A6 (§1.6) — a validated re-derivation is a capture moment.** Two classes of writer, one
  honesty invariant (§7.0). **Scopes SNAP-AC-1's carry-forward half, SNAP-AC-9's application and
  SNAP-AC-16.**
...
validation rule gone, the WERS reference field still editable.
...
The diff changes no file under `worker/lib/staff.ts`, no role CHECK constraint, no migration,
and **no application save path other than the JSON snapshot builders whose source fields this
phase deletes** — `proposal.ts:375` (written at `:415`) and `splitCandidates.ts:356`.
...
change, **no statement's column list or WHERE clause altered**. The breach is confined to the
**contents of a JSON blob**.
...
**Why this is its own phase.** **Not urgency** — see §4's retraction. It is a **server
write-path change with the widest blast radius in this feature**, it falls under Probity, and
§7.2's negative criteria are worth proving against a deployment in which nothing else moved.
...
- **Best-effort writers** — they **ask** the catalogue and must succeed even when it cannot
  answer. **They have a failure channel**, so a re-resolve on an unmoved pick can only degrade
  or lie. **A4's predicate governs them**: carry when unmoved, resolve fresh when moved.
- **Derivation writers** — they **are** the derivation: the figures come from the same validated
  or estimator-selected in-memory variant that produces the product, the price and both
  snapshots **in the same statement**. **They have no failure channel that can write a dishonest
  absence** — the path refuses before writing when the catalogue will not answer, and any
  absence they write is the deliberate record of an evaluation that happened. **For them the
  pick-moved predicate is the wrong condition**: a dimension or quantity edit does not move the
  pick but must reprice, and carrying figures forward while `line_total`,
...
has already grown once**: A6's re-walk found two sites nobody had questioned. **Verified at 3a:
the tester traced this property against every member and could not construct a violating path.**
...
That is **the branch's pre-existing contract**, and whether non-material ops edits should
re-validate at all is a **product question about that branch**, not a figures defect (§14).
...
> `selected_variant_id`** (`projects.ts:579`; `pickMoved`'s `!!pick.variantId &&` clause keeps it
> stable rather than churning it on every glass swap). **The row therefore stores a variant that
> disagrees with its figures, and that is ruled, not accidental** — design §4.3, and SNAP-AC-14's
> whole intent is to record what the customer *chose*. W11 (segment edit) has the same shape.
>
> **The old wording described the wrong thing** and would have made a correct write look like a
> breach — §12 note 13's third instance, again two criteria over one subject.
>
> **The consequence is a Phase 3b copy rule, not a data change: WHY-AC-4.** The stored
> `selected_variant_id` is **not the caption for the figures**. A panel that says *"this variant
> performs at X"* beside an id meaning something else is the copy problem this note exists to
> prevent.
>
> **No opportunistic backfill on touch.** A pre-capture line edited by a best-effort writer does
> not quietly acquire figures (SNAP-AC-10's reasoning).
...
*Neither A4 nor A6 weakens this:* every scanned statement still names the column.
...
save that moved the pick** to a configuration the catalogue cannot answer for — no matching
published variant, or unreachable — *Then* the save completes, the figures are stored as
...
stored figures would otherwise describe a configuration the row no longer has.
...
2. a save that **successfully resolved** a published variant which genuinely carries no figures
   — the answer was established, and the answer is "none recorded";
...
> **Read it against the invariant, not against a list of sites (§7.0).** What this forbids is
> **asserting an absence you did not establish**. **A tester must classify the writer first** —
> and if a writer's class is unclear, that is a design question, **not a licence to weaken the
> criterion at the site where it is inconvenient**.
...
A hand-maintained list has been **incomplete on every attempt**: named 1, then 4, then 8,
verified at **15** — **and the list layer failed a third time in a different direction** when
A6's re-walk found two writers in a class nobody had enumerated. **Write criteria against
properties, and let the design carry the membership.**
...
priced records the figures of **what the customer chose**, so WHY-AC-27's comparison is available
on precisely the lines stamped `customerConfigurationChanged`. **This is the intent SNAP-AC-1's
...
> criterion go red. **The criteria stay exactly as written: they are what the next reader must
> not break.**
...
push was rejected by name:** the address bar would lie, and reload and shared links would
silently lose the viewer.

**VIEW-AC-1 (R21, R25)** — the viewer opens carrying **the whole drawing and its caption visible
together, the drawing as large as the viewport permits** — and nothing that explains the
drawing's notation.
...
> > large as the viewport permits.**
>
> **Which dimension governs: whichever one binds.** As shipped it was height alone
> (`height: 62vh`), measured with each dimension moved separately:
>
> ```
...
> MINOR. **Verified by three shapes, each moving one dimension**; the round-1 test grew both
> together and passed whichever dimension was doing the work (§12 note 12).

**VIEW-AC-1a (D10)** — a **unit** shows its code; the **line** is titled `Drawing`.

**VIEW-AC-2 (R31)** — **exactly one** history entry, and the address becomes that drawing's URL.

**VIEW-AC-2a (R31)** — back control, Escape and the system gesture all do the **same single
pop**; **not remounted**, **no record re-fetch**. *(From the canvas the destination is the
record — VIEW-AC-14.)*

**VIEW-AC-2b (cold deep link)** — renders the line page **with the viewer open**; back
**replaces** to the line path.

**VIEW-AC-2c** — a malformed or out-of-range suffix **normalises by replace** and **grows no
history**.

**VIEW-AC-2d (R31)** — two backs to leave a line. **Agreed cost, not a defect.** Nobody may
collapse the two steps.

**VIEW-AC-3 / VIEW-AC-4 (composites)** — the whole assembly in proportion, no explaining
sentence; a single unit alone at `…/drawing/u<N>`, titled with its code, captioned with its own
size.

**VIEW-AC-5 (R21)** — exactly one viewer component, and **every** enlargeable drawing opens it —
**the desk canvas included**.

**VIEW-AC-6** — the previous `SidePanel` enlargement no longer appears anywhere.

**VIEW-AC-7 (keyboard)** — Enter/Space opens it, focus moves in, the accessible name carries the
subject, and back returns focus to the drawing that opened it.
...
styles mean, no sentence teaching that panel widths are proportional. **Must not rest on the
`.elev-legend` selector** — §12 note 5.

**VIEW-AC-11** — `ops2-record-correction.md`'s **P1-AC-27** is **marked superseded**, naming this
feature and R25.

**VIEW-AC-12 (the shared export IS deleted) — REVERSED by the owner (D12).** Export and body
removed, the pinning assertion with it, no live-source reference, ADR 0010 annotated.
...
architect-conformance time to satisfy VIEW-AC-5 — §12 note 11.

**VIEW-AC-13** — activating the canvas's plate or a unit row opens the **shared** viewer at the
line's own drawing URL, **same grammar**, **exactly one** history entry.

**VIEW-AC-14** — back, Escape and the system gesture all pop once to the **record** — never to
the line page they never visited. *Cold arrival unchanged (VIEW-AC-2b).*
...
**Why the reference (§13.17 VETOED).** `LinePage.tsx:201` already names this destination by
reference. **One destination must not have two vocabularies in one console.**

**There is no `Project` fallback on the viewer's control, and that is not a descope.** Its
subject is built only once the record has resolved a line. **The page-level control keeps
`Project`**; **if one of these two labels ever changes, both change**. **The never-empty
guarantee is pinned, not assumed** (`record.ts:314`; `parseProjectRecord` returns `null` without
a non-empty id) — *the pinned case is the absent field; empty and whitespace rest on `str()`'s
trim (§12 note 7)*.

**VIEW-AC-16** — the record is **not remounted and not re-fetched**, the selected line is
**still selected**, and the canvas shows its drawing **from the first frame after the pop**.

**VIEW-AC-17** — `scripts/tests/web/` carries **executed** Playwright coverage of the journey.
**A node:test suite cannot satisfy this criterion.**
...
them a platform default; equivalently `explicit_energy_report`, `plan_derived`,
`human_override`.

**WHY-AC-3 (R4, absent)** — "Had to meet" says this opening had no thermal requirement, and no
cap figure is shown.
...
row's `selected_variant_id`, and never phrased as *"this variant performs at X"*. A null figure
is stated as not recorded and never rendered as a number, a zero or a dash.

> **Why the attribution rule exists.** After a customer glazing-only change the row's
> `selected_variant_id` is deliberately **left as the estimator's** while the figures describe
> what the glass resolved to (SNAP-AC-1's note, design §4.3). **The id is not the caption for the
> figures.** Copy that reads the id and captions the numbers with it would state something the
> row does not mean — on exactly the lines a reviewer is most likely to be auditing.
...
**WHY-AC-7a (R29, D8)** — a **back** control with an accessible name, plus the standard back
gesture; **no "Done" and no X.**

**WHY-AC-7c / 7d (D8 — cold arrival)** — the detail renders its line's rationale, resolved the
way the line page does, with the line page's not-found and refusal sentences; back **replaces**
to the line page. Never a dead control, never out of the console.

**WHY-AC-7b (negative)** — the Projects filter panel is unchanged, tests included.
...
**WHY-AC-9 (D5, the honest gap — and present-and-null has two meanings, revision 25)** — *Given*
a line whose stored figures are **absent** (`NULL` — saved before the capture existed), *Then*
the panel states that a person chose this product and that its figures were not recorded, and
**no** catalogue lookup fills the gap. `ASSUMED:` §13.5.

> **`NULL` and present-and-null are different sentences, and present-and-null has two causes.**
> SNAP-AC-8 glosses present-and-null as *"no figure exists for this product"* — **which is not
> the only thing it can mean.** A derivation writer records *"evaluated, nothing chosen"* the
> same way, and one of them (`proposal.ts:262`) **nulls the variant while leaving `product_slug`
> standing**, so a later reader sees product X with present-and-null figures. Inside the
> statement it is coherent (`line_total=NULL`, `status='incomplete'` say plainly that nothing was
> selected) — **but the panel will render it, and copy that assumes the first meaning will
> mis-state the second.**
>
> **The 3b copy must distinguish them from the row, not from the figures alone**: a line with no
> selection is not a line whose product has no published figure. This is recorded here rather
> than discovered when the panel ships — the variant-nulling is pre-existing and is not a 3a
> defect.
...
**WHY-AC-12 (R8)** — chosen first, marked as chosen, then the next four by ascending ladder rank
— five rows, no more. **WHY-AC-13** — fewer than five: the ones that exist, no placeholders, **no
count of anything beyond the list**.

**WHY-AC-14 / WHY-AC-15 (R9 — negative)** — no excluded candidate, no count, no exclusion reason
text; nothing about withheld products.
...
**WHY-AC-20 (R1 — negative)** — no control changes anything, and a network trace contains only
GETs.
...
**WHY-AC-22 / WHY-AC-23** — the platform's original recommendation shown **unchanged**, with a
comparison of the current product against the **same** requirement beside it; variant/glazing
differing counts as a change.

**WHY-AC-24 (negative)** — nothing changed: no human-selection block at all.

**WHY-AC-25 / WHY-AC-26 (R23)** — the requirement shown is exactly the one recorded at selection
time, and **no code path re-resolves, recomputes or re-derives a requirement**.

**WHY-AC-27 (D3, D5)** — the comparison uses the **stored** figures against the run's recorded
caps, in WHY-AC-17's vocabulary. Figures absent: it says so and makes no live lookup.
...
**WHY-AC-30 / WHY-AC-31 (R24)** — a restore reads as platform-made once more by the same
comparison; no code path writes `origin` or `ai_proposal_line_id`.
...
stored home: `quote_line.review_json.composite`. If staff have resolved that flag the trace is
gone — a named residual; the fixture must carry the unresolved flag.)*

### 9.5 No action, anywhere on this surface (R28, R1, R29)

**That absence is the design, not a gap.** Owner: *"Having a button implies that some product
must be preselected, which we don't have conceptually."*
...
unimplementable. The criteria assert **one refusal for every non-staff caller** and put the
weight on **no candidate data in the raw response body**.
...
at most four runners-up.

**X-AC-6** — this feature adds **no** POST, PATCH, PUT or DELETE endpoint.

**X-AC-7** — no schedule comment text, no data belonging to any other opening, project or
account.
...
id from another project unchanged.
...
> `ops.ts:953-966` maps the body through an **explicit whitelist** (`widthMm`, `heightMm`,
> `productSlug`, `qtyPerParent`, `options`), so the fields above are never read and W10's snapshot
> branch is unreachable from a client. **But that safety lives in a list**, and this repo's list
> layers have been wrong repeatedly (§7.4: 1→4→8→15, plus a whole class nobody had drawn). **A
> refactor that widens the mapping should fail a test, not depend on the next reader noticing.**
> Executed as a real attempt, like X-AC-8/9/10 — inspection is what it replaces.
...
| **An ops edit of an AI-priced line where nothing material changed** | Price, snapshots **and** figures refresh. **Named residual, pre-existing contract.** A product question about that branch. | A6, §14 |
...
| **A line whose variant was nulled but whose product still stands** (`proposal.ts:262`) | Present-and-null figures mean **"no selection was made"**, not "this product has no figure". The row says so plainly (`line_total=NULL`, `status='incomplete'`); **3b's copy must read the row, not the figures alone.** | WHY-AC-9, SNAP-AC-8 |
| **A save that genuinely moves the pick while the catalogue is unreachable** | Present-and-null, and **here it is the honest fact**. Save completes; nobody is told. | SNAP-AC-6 |
| **A successful resolution of a published variant that carries no figures** | Present-and-null, **honestly** — the answer was established and the answer is "none recorded". | SNAP-AC-16 |
| **A pre-capture line that a best-effort writer edits** | Stays `NULL`. **No opportunistic backfill.** | A4, SNAP-AC-10 |
| **A product offering exactly one published variant, with no variant named** | Resolves to that variant — **a singleton answer set is not ambiguity**. Ambiguity begins at two. | A5 |
...
| **A project served without a `public_ref`** | The back control shows the 36-character id, **capped as a display problem**. Never the project's title. | VIEW-AC-15 |
| **Leaving a line after enlarging a drawing** | **Two backs.** Agreed cost of R31. **Not to be collapsed.** | VIEW-AC-2d |
...
| **The catalogue is unreachable at save time** | Save completes, nobody is told. **On an unmoved pick nothing is written; at a derivation writer the path refuses before writing anything.** | D6, SNAP-AC-16 |
...
2. **SNAP-AC-2 is a source-level scan.** Property of every match, never a count; fails when its
   own match set is empty. **A1's dedicated column is what makes it one regex.**
3. **SNAP-AC-5 and SNAP-AC-15 need a customer-path test** — where the widest defect nearly
   shipped.
4. **X-AC-1 must be executed for both callers separately.** **And X-AC-13 must be an executed
   attempt, not an inspection** — it exists precisely because inspection is what covers it today.
5. **VIEW-AC-10, and the selector that became a trap.** Once `ElevationLegend` was deleted, a
   `.elev-legend` `toHaveCount(0)` became true by construction.
6. **WHY-AC-39/40/41 assert absences.** Enumerate and assert the *set*; WHY-AC-40 is a
   route-table assertion.
7. **WHY-AC-7a must assert where back GOES, not what it looks like**; **VIEW-AC-15 adds the other
   half** — assert label and destination **together**. **Two controls, two states.** And the
   never-empty guarantee is **only half pinned**.
8. **VIEW-AC-2's numbers are the assertions.** "Exactly one" pushed, "one pop" from all three
   exits.
...
10. **A JUSTIFICATION IS A CLAIM, AND IT MUST BE CHECKED — six times about the codebase, once
    ...
    `api.ts:381` exemption; VIEW-AC-12's premise; "exactly one reference repo-wide" (**a count is
    a claim too**); *"removes a render, not an API"*; and **"there is no precedent"**, the one
    nobody thinks to grep.

    > **A claim about the codebase is settled by running something. A claim about the world is
    > settled by asking the person who owns it** — in the same breath as the decision it is being
    > used to justify.

    **The tell:** "another consumer", "a legacy surface", "everywhere else", "somebody might use
    it", "every save in between", "still open", "nobody has ruled on this".
11. **A journey that no criterion names will be tested by nobody** — and it is a later stage that
    adds them. **When a review stage adds behaviour, the criterion is part of the fix.**
12. **A two-variable measurement proves nothing about which variable did the work.**
13. **A CRITERION IS CHECKED AGAINST THE OTHER CRITERIA IT CONSTRAINS, NOT ONLY AGAINST THE CODE
    — two of them, each correct alone, can be jointly unsatisfiable. THREE INSTANCES NOW, AND
    NONE WAS FOUND BY READING.**

    - **VIEW-AC-1 (found by a measurement).** "Grow with the width" and "the caption stays
      visible" cannot both hold on a short wide screen.
    - **SNAP-AC-1 vs SNAP-AC-9 (found by an implementation).** An implementation satisfying the
      first as written **violates the second** — and one did, silently, on the most common save in
      the product.
    - **SNAP-AC-1 vs SNAP-AC-14 (found by a tester tracing a path, revision 25).** SNAP-AC-1 said
      the figures are "that product+variant's"; SNAP-AC-14 requires recording **what the customer
      chose**, which on a glazing-only change is a glass and not a variant — while the row keeps
      its old variant id by design. **Correct code, described wrongly by a criterion.**

    **All three had a shared subject** — the drawing's size, the stored figures, the stored
    figures again. **Two criteria over one subject is where to look**, and there is no substitute
    for looking: no test catches a contradiction, because the failing implementation is
    *conformant to one of them*. **Review reads criteria one at a time; contradictions only
    surface when something has to obey both at once.**

    *(All three repairs preserved the criterion that was right. When two criteria collide, one is
...
14. **A CRITERION GENERALISED FROM ONE INSTANCE MUST NAME THE PROPERTY THAT MADE THAT INSTANCE
    ...
    SNAP-AC-16 was written from a single defect and it worked — it is the criterion the
    architect's re-walk tested every writer against, and it **found two more sites**. Then it
    **over-caught**: both are correct code.

    *"Present-and-null has exactly one author"* describes **a behaviour**, and behaviours have
    honest instances the author never saw. The property was: **no writer may assert an absence it
    did not establish.**
    ...
    **Why this is more dangerous than a stale bullet, and nearly its inverse:** a criterion that
    ...
    implementer who cannot satisfy it** — and the pressure at that moment is to *quietly weaken it
    at the one inconvenient site*, leaving the criterion looking intact and the rule silently
    gone. **The exemption must be granted by the spec, not assumed by the code.**

    **The check when writing one:** ask *"what made the instance I saw wrong?"* If the answer is a
    sentence about **what the code did**, it is a behaviour and it will over-catch; if it is about
    **what the code could not know**, it is a property.
...
**literal two-file array**. **SNAP-AC-2's scan inherits this exactly**, and **X-AC-13 exists
because a whitelist is the same layer.**
...
had already ruled both**, in a design section which names the entry numbers it overrides.
...
> register.** An entry that says *"the architect may rule otherwise"* creates an obligation with
> no return address.
...
| §7.2 | the `CandidateOutcome.dataSource` removal is the architect's to rule | **DISCHARGED — architect (A3)**; ADR 0011; CERT-AC-9 protects stored rows |
...
| 5 | **WHY-AC-9** — where figures were never captured, the panel says so | **OPEN** — Phase 3b. **Revision 25 adds the second meaning of present-and-null to this entry's neighbourhood** — a veto here now has more copy behind it |
| 6 | *(the capture's reach)* | **RETIRED** — D7 |
| 7 | *(where "Change the product" lives)* | **RETIRED** — R28 |
| 8 | **SNAP-AC-12** — figures live in `configuration_snapshot_json` | **DISCHARGED — architect (A1): OVERRIDDEN.** A **new nullable column**. **Why the assumed home was wrong:** that column is deliberately **set to NULL** when a customer materially edits an AI-priced line — so the save that most needs to *record* figures is the save that *erases* it. **Content is exactly `{"uValue", "shgc"}`** — duplicating the variant identity would create **a second home for a fact** |
...
| 19 | *(the 3a-before-3b urgency)* | **VETOED — owner (D14).** Asserted as fact in prose; registered retrospectively so the correction is visible where the states are read |
...
| 21 | *(SNAP-AC-16's "exactly one author", read as governing every writer)* | **VETOED — architect (A6).** The opposite failure to §13.20: **too strong**, condemning correct writers. Scoped to best-effort writers, with derivation writers named **as a class with a property** |
| 22 | *(SNAP-AC-1's "that product+variant's figures", read as a promise that the row's `selected_variant_id` describes them)* | **VETOED — tester's trace, revision 25.** On a customer glazing-only change the figures come from the glass and the row keeps its old variant id **by design** (design §4.3, `projects.ts:579`). Wording corrected; **the substance goes to the owner at acceptance** (§14), and the copy consequence is pinned in **WHY-AC-4** |
...
estimator set**, rather than churning it on every glass swap. So the row can hold a variant id
that does not match its figures. **That is deliberate** — design §4.3, and it is exactly
SNAP-AC-14's intent: record what the customer *chose*.
...
because *"this variant performs at X"* beside an id meaning something else would mis-state the
row on exactly the lines a reviewer is most likely auditing.

**The alternatives, both worse:** null the variant id on every glazing change (churn on a field
other things read), or re-resolve a variant id to match the figures (a guess written into a
column that means "what was selected").
...
**What else moved in revision 25.** **X-AC-13** is new: the split path's body whitelist becomes an
executed attempt rather than an inspection — safe today, but **a whitelist is a list layer, and
this repo's list layers have been wrong repeatedly**. **WHY-AC-9** now records that
present-and-null has **two** meanings, so 3b's copy distinguishes *"no selection was made"* from
*"no figure exists for this product"* by reading the row rather than the figures alone. **§12 note
13 gains its third instance**, and §13.22 registers the corrected reading.
