...
**Date:** 2026-08-25 · **Stage:** pipeline stage 1 (product-manager) · **Revision 30**
...
`72d6b28b`; **not yet deployed, by the owner's choice** (D17). **Phase 3b gate closed** (D18–D20);
**3b is built and green** — pending ui-designer, tester and acceptance.

**Revision 30 walks §9 against the approved mock, which nobody had done — and it disagreed in six
places.** One was reported to me; **five were found by the walk.** The mock is
`docs/mocks/ops2-why-this-product.html`, and it is **the only description of this surface the owner
has actually signed**.

| Mock | What it shows | What §9 said |
|---|---|---|
| **B8** | `unrecorded` still shows **"This one"** with the line's figures | Silent — and the DTO omitted `current` |
| **B5** | An ops split says **"These ones"**, lists **per-unit** figures, and cuts above three with **"+2 more units"** | *"two lines… that product's own Uw and SHGC"* — singular, wrong label, no overflow rule |
| **B3 / B12** | **"This one"** carries a qualifier — *glazing changed*, *frame and glazing changed* | *"only the third line's sentence changes"* |
| **B4** | A composite parent's **"This one"** names the make-up, no figures | WHY-AC-4 would have rendered a null-figure sentence |
| **B11** | Requirement absent → **"the cheapest that fitted the opening"** | WHY-AC-5's five tier sentences do not include it |
| **failure state** | A failed read **keeps the panel, says the read failed, offers Try again — and never renders as "not recorded"** | **No criterion at all** |

**The last is the most important, and the mock states the invariant better than this spec did:**
*a missing fact and an unreachable one are different things, and only one of them is worth
retrying.* That is A6's honesty invariant on the read side — **never assert an absence you did not
establish** — and §9 had no criterion for it. **WHY-AC-42 is new.**

**Also corrected: X-AC-3 named a gate that does not fire.** `resolveStaff` refuses a manufacturer
first, so the body is `forbidden`, not `forbidden_role` — **which is stronger than the criterion
described**, and the reason is now recorded so nobody "harmonises" it away.

**§12 note 17 is new:** **a criterion, a type and a mock can each be internally consistent and still
disagree — and only one of the three has the owner's signature on it.**

**Revision 29** amended WHY-AC-29 to compare **the pick** after D16 made product+variant wrong.
*(Mutation-proved by the developer: reverting to product+variant goes red on the D16 case alone —
the fixture §12 note 1 names as the only one that distinguishes the two wordings.)*
**Revision 28** closed the 3b gate and repaired WHY-AC-27 and WHY-AC-4. **Revision 27** recorded
D15–D17. **Revision 26** recorded the phase's lesson (note 15). **Revision 25** folded in three
tester observations. **Revision 24** scoped SNAP-AC-16 (A6). **Revision 23** repaired SNAP-AC-1 vs
SNAP-AC-9 (A4). **Revision 22** reconciled the register against the design. **Revision 21**
retracted a false urgency. **Revision 20** recorded the assertion-that-cannot-fail rule; **19**
corrected VIEW-AC-15; **18** resolved VIEW-AC-1's internal contradiction; **17** carried the veto of
§13.17; **16** carried D11 and D12.
...
VIEW-AC-2's mechanism; revision 10 folded in D8 and R31; revision 9 applied R29/R30 and added §2.1;
revision 8 applied R28; revision 7 corrected a false claim about an existing legend test; revision 6
applied the closed UX mock gate; revision 5 repaired two architect findings; revisions 2–4 folded in
the owner's decision rounds.

**Decisions needed: one** (§14) — whether the record's desk canvas shows the panel. **It needs a
criterion before it is built either way.**
...
machine's word or re-does the thermal comparison by hand. Neither is a review. In the owner's words:
*"I want to be able to audit recommendations and accuracy of thermal modelling"* and *"the bigger
vision is to surface most likely alternatives hopefully making human's work easier."*
...
downgrades **13 of 32 products to "indicative estimate only" on every thermally-constrained line**.
**And the catalogue importers write the flag back on every run** (§6, CERT-AC-12).

And a third gap, **closed by Phase 3a**: a product's thermal performance was not recorded on the
line that uses it. Under R3/D3 nothing may look it up at display time to recover it.
...
1. **The read surface.** A three-line "Why this product" panel on the line detail view, and the
   detail **screen** it leads to — its own route (D8) — carrying the requirement and its origin, the
   chosen product against it, why it won, and **four** next-best alternatives by ladder rank (D18).
   Composites included (R14–R17). A **thinner panel** for lines nobody's estimator ever evaluated
   (D6). **No action anywhere on it** (R28) — **except the failure state's retry** (WHY-AC-42),
   which acts on the read, never on the line.
...
| **Any change to the ops routes' authentication or refusal convention** | §10 — which is why §13.11 is RETIRED. **X-AC-3's correction is a description of what already fires, not a change.** |
...
both**; **D19** applies it to gaps; **WHY-AC-42** applies it to failure — *a reviewer told "not
recorded" stops looking. A reviewer told "could not be read" tries again.* **The panel must never
turn the second into the first.**

**The one thing this reader is actually auditing is the attribution.** *"Did the platform choose
this, or did a person?"* **WHY-AC-29 is that answer's mechanism**, and a stale identifier must never
...
**Their save must never start failing** (§7.2), and **an autosave that changes nothing about the
pick must not touch their figures** (A4, SNAP-AC-16).
...
Refused before the route runs. **This surface exposes which competing products were considered and
how they compared**; §10 executes it as an abuse case. **X-AC-3 records which gate actually fires.**
...
- **Phase 3b — a read-only display surface (§9)** · **built and green**; ui-designer, tester and
...
> between** — which is what makes one deploy event safer than two.
...
R9 no excluded · R10 no price deltas · R11 human change shows both · R12 frame or glazing = changed ·
R13 no-run lines still show the panel · R14 panel on machine-proposed composites · R15 split reason
then per-lite bands · R16 per-lite bands are real · R17 ops split = R13 · R18 leave history · R19
...
save, never read live · ~~**D4**~~ · **D5** the capture waits for Phase 3 — ***about lines saved
BEFORE THE CAPTURE EXISTED*** · **D6** a client/manual-picked line shows a thinner panel **with** its
product's figures · **D7** the capture extends to the customer save path · **D8** the detail screen
gets its own URL · **D9** the URL grammar · **D10** the viewer's title names the subject · **D11**
back from the canvas returns to the record · **D12** `ElevationLegend` is deleted · **D13** the
drawing claims all leftover space · **D14** the phasing is about manageability, not urgency · **D15**
Phase 3a accepted · **D17** 3a and 3b deploy together.
...
`selected_variant_id`**. The owner accepted the consequence **on the condition that nothing
downstream reads that id as the answer.**

> **Three criteria carry that condition. If any one is weakened, D16 re-opens.**
> 1. **WHY-AC-4** — never **captions the figures** with that id.
> 2. **WHY-AC-29** — never **decides the attribution** on it (amended revision 29; compares **the
>    pick**, §7.0). **Mutation-proved: reverting to product+variant goes red on the D16 case alone.**
> 3. **The DTO omits `current.variantId` entirely** — the forbidden reads are **impossible rather
>    than merely forbidden** (§12 note 16).

**D18 — panel scope.** Chosen product **plus four alternatives, five rows**; **withheld products
hidden**; **no money anywhere**. Discharges §7.3, §7.1, §13.3.

**D19 — thin, old and plural records.** The panel **shows what was recorded and says so plainly when
there is nothing; it never reaches back to fill a gap.** Discharges §13.5, §13.2, §13.4.

**D20 — the overridden line.** **The same three lines everywhere**; the **"Chosen" sentence** carries
the attribution. *(Revision 30: the mock also marks **what** changed on "This one" — WHY-AC-28.)*
...
or `energyCertified`; no `certified` as a field name or written value; and no `dataSource` **where it
is valued** `"certified" | "estimated" | "manufacturer"` or written/projected on a performance
...
blast radius in this feature**, and §7.2's negative criteria are worth proving against a deployment
...
> **The invariant has a read-side twin, added revision 30: WHY-AC-42.** A writer must not record an
> absence it did not establish; **a reader must not display one either.** A failed fetch is not a
> missing fact.
...
`performance_figures_json`. **No count encoded; cannot pass vacuously.**
...
**SNAP-AC-4 / SNAP-AC-5** — an ops edit and a customer save that succeed today still succeed; **no
new validation, 409 or error path**.

**SNAP-AC-6 (best-effort resolution)** — a best-effort save that moved the pick to a configuration
the catalogue cannot answer for completes, stores **present-and-null**, tells nobody.
...
**D19 confirms the same refusal on the read side.**
...
ordinals**. **A state-only history push was rejected by name.**
...
deep link **replaces** to the line path; a malformed suffix **normalises by replace**; leaving a line
after enlarging takes **two backs** — **agreed cost, not to be collapsed**.
...
**VIEW-AC-13** — the canvas opens the **shared** viewer at the line's own drawing URL, **exactly
one** history entry.
...
> **Gate closed (D18–D20).** **Four criteria have been repaired since they were written** — WHY-AC-27
> and WHY-AC-4 (rev 28), WHY-AC-29 (rev 29), and **five more in revision 30's walk against the
> approved mock**, which also added **WHY-AC-42**. §12 notes 13 and 17 carry the classes.
...
panel appears between the specification (or units block) and the price, carrying exactly three lines:
**Had to meet**, **This one**, **Chosen**.

**WHY-AC-2 (R4)** — `default_envelope` states the caps as figures **and** an origin label naming them
a platform default; equivalently `explicit_energy_report`, `plan_derived`, `human_override`.
...
dependency; composite case added revision 30)** — *Given* a line whose stored figures **are
numbers**, *Then* "This one" shows the Uw and SHGC **from the line's own record**, attributed to
**the product and the glass they describe** — never to the row's `selected_variant_id`, and never
phrased as *"this variant performs at X"*.
...
*Given* a **composite parent**, which has no product and no figures of its own (§7.4), *Then* "This
one" **names the make-up** — *"made as 2 units — an awning beside a fixed pane"* — and **no absence
sentence is shown**, because nothing about the parent is missing. *(Mock B4.)*

> **This criterion is part of the consideration the owner was given for D16** — one of **three**.
> **Weakening any of the three re-opens D16.**

**WHY-AC-5 (R6, R7; absent-requirement sentence added revision 30)** — *Given* the run's
`competingTier`, *Then* "Chosen" states the winning rule in one sentence per tier: `meets` → the
cheapest of those that met the caps; `within_tolerance` → nothing met the caps, so the cheapest
within 5% of the closest; `misses` → nothing came within the band, so the closest was taken;
`thermal_unknown` → no figure existed on the constrained axis; `does_not_fit` → nothing fitted, so
the best fit was taken.

*Given* the requirement was **absent**, *Then* the sentence is **about fit and price only** — *"the
cheapest that fitted the opening"* — and **claims no thermal victory**, because with no caps every
candidate is in `meets` by definition. *(Mock B11; §11's edge case demanded this and the five tier
sentences did not supply it.)*

**WHY-AC-6 (R7)** — a run stored with `tolerance` 0.08 says 8% — read from the run, never hardcoded.
...
rationale with the line page's not-found and refusal sentences and back **replaces** to the line
page; the Projects filter panel is unchanged.
...
product, and that product's own Uw and SHGC. No "Had to meet", no "Chosen" rationale, **no
alternatives action**.
...
*Given* **present-and-null**, *Then* the panel distinguishes the two causes **from the row**: where
the row shows a selection, the product has **no published figure**; where the row shows **no
selection was made**, it says that instead.
...
> vacuous `every()` over that empty set is **true** — sending the line to *"reasoning not recorded"*,
> the wrong sentence. **See §12's opening rule: the empty case satisfies the check.**

**WHY-AC-10 (R3, D19, pre-0055 rows; "This one" added revision 30)** — *Given* a line whose selection
run predates `outcome_json`, *Then* the "Chosen" line states that this recommendation was **recorded
by an earlier model whose reasoning was not kept**; **no reconstruction**, and no alternatives
action.

**"This one" still renders from the line's own record.** *(Mock B8.)* **The age of the run has
nothing to do with the figures**: they come from the line (A1, SNAP-AC-3), not from the run. **An
unrecorded run means we cannot say *why*; it does not mean we cannot say *what this performs at*.**

> **This is where §9, the DTO and the mock disagreed.** The design's `{ kind: "unrecorded" }` carried
> no `current`, so the figures were unreachable; the mock showed them; §9 was silent and settled
> nothing. **The mock is right and the DTO must carry `current` for this kind** — see §12 note 17.
...
**WHY-AC-16 (R10, D18 — negative, whole surface)** — **no price, no price delta, no currency symbol
and no control that prices anything**, on the panel or the detail.
...
**WHY-AC-18 (R3, D3, D19)** — every figure shown is a stored one; **no request to the catalogue or
...
trace contains only GETs. *(WHY-AC-42's retry re-issues the same GET — it changes nothing.)*
...
**WHY-AC-27 (D3, D5, §9.0 — repaired revision 28)** — *Given* stored figures that **are numbers**,
the comparison uses them against the run's recorded caps. *Given* they are **not numbers**, the
comparison is **not attempted**, the panel says which absence it is **in §9.0's terms**, and it still
shows the requirement and the platform's recommendation unchanged, with **no live lookup**.
...
**WHY-AC-28 (R24, D20 — attribution, the sentence; qualifier added revision 30)** — *Given* a line
whose current selection differs from the configuration the platform recommended, *Then* **R6's three
labels are kept** and the **"Chosen"** sentence carries the attribution: a person chose this product,
and what the platform had recommended instead.

**And "This one" carries a qualifier naming *what* changed** — *"glazing changed"*, *"frame and
glazing changed"*. *(Mock B3, B12.)* **R12 makes that distinction meaningful**, and a reviewer who
can see only *that* something changed has to go and find out *what*.

> **D20 fixed the structure, not the silence.** *"Only the Chosen sentence changes"* was about
> keeping three labels rather than growing a fourth line; **it was never a rule that the other two
> lines may say nothing new.** The mock the owner approved shows the qualifier, and §9 as written
> forbade it.

**WHY-AC-29 (R24, §7.0, D16 — attribution, the MECHANISM; amended revision 29)** — *Given* a line the
estimator created (`origin = 'ai'`, `ai_proposal_line_id` set) whose configuration a customer has
since overridden, *Then* the panel says **a person chose this product**.
...
only where both sides name one**. Never from `origin`, never from `ai_proposal_line_id`, and **never
from `selected_variant_id` alone**.

**Two fixtures, and the criterion is not met by one of them:** the product changed (any comparison
catches it); and **only the glass changed** — product matches, **the row still carries the
estimator's `selected_variant_id`** (D16), so **a product+variant comparison concludes
*platform-made***.

> **~~As written through revision 28:~~** *"comparing… product+variant"*. **Correct when written;
> wrong after D16.** **The panel would have credited the platform with the customer's own choice —
> on exactly the lines stamped `customerConfigurationChanged`.** **The glazing term dominates
> precisely where D16 leaves the id stale.**
>
> **Mutation-proved by the developer:** reverting to product+variant goes red **on the D16 case
> alone** — the fixture §12 note 1 names as the only one that distinguishes the two wordings.

**WHY-AC-30 / 31 (R24)** — a restore reads as platform-made once more by the same comparison; no code
path writes `origin` or `ai_proposal_line_id`.
...
**WHY-AC-33 (R15)** — the detail shows the split reason first — the make-up that won and the single
unit it beat — then each lite's own band.
...
**WHY-AC-37 (R17, D6; shape corrected revision 30)** — *Given* a composite whose `composite_origin`
is `'ops'`, *Then* the panel shows **two lines and no alternatives action**: **"These ones"** — each
unit's code with its own Uw and SHGC — and **"Chosen": a person decided this split**. No "Had to
meet", no machine rationale, no split reason.

**Above three units the figures line cuts and says so** — *"+2 more units"*. *(Mock B5.)*

> **WHY-AC-8's treatment was the wrong shape for this case.** It says *"that product's own Uw and
> SHGC"* — **singular** — but an ops split has no product of its own and several units that do.
> **The label differs too** (*"These ones"*), and the overflow rule existed only in the mock.
...
**WHY-AC-41** — the panel's only interactive element is the one that opens the detail **(and, in the
failure state only, the retry — WHY-AC-42)**.

### 9.6 The read's own states (new, revision 30 — from the approved mock)

**WHY-AC-42 (A6's invariant on the read side; the mock's failure and loading states)** —

*Given* the rationale read is **in flight**, *Then* the panel is present and shows **skeleton lines
in the shape it is about to be**, so the page does not jump when the answer arrives.

*Given* the rationale read **fails** — network, timeout, a 5xx, anything short of a refusal — *Then*
the panel **remains**, says **the reasoning for this line could not be read just now**, and offers a
**retry**. It **never renders as "not recorded"**, never as an empty panel, and **never disappears**.
**The rest of the line page is unaffected.**

*Given* the read is **refused** (WHY-AC-11's post-issue case, or a line with no rationale), *Then*
that is **not a failure** — it takes the refusal sentence, and no retry is offered.

> **This is A6's honesty invariant, on the read side, and the mock states it better than this spec
> did:** *"a missing fact and an unreachable one are different things, and only one of them is worth
> retrying."*
>
> **The business consequence is the whole point.** **A reviewer told "not recorded" stops looking** —
> they conclude the platform never recorded a reason and move on, possibly confirming a
> recommendation they could have audited. **A reviewer told "could not be read" tries again.**
> Collapsing the second into the first is **a false absence generated at display time**, which is
> exactly what SNAP-AC-16 forbids the writers from doing.
>
> **The retry is not an action on the line** (R28, R1): it re-issues the same GET and changes
> nothing (WHY-AC-20). It is the one interactive element §9.5 admits beyond the panel's own action.
>
> **§9 had no criterion for any of this** — the mock had it, the design had it, and the spec did
> not. See §12 note 17.
...
**X-AC-3 (the control that matters most — conclusions §1, R20; gate corrected revision 30)** —
*Given* a signed-in user whose staff role is `manufacturer`, *When* they request the rationale —
**the endpoint and the `why` URL both** — *Then* they are **refused**, and the raw body contains no
competing product slug, tier, figure or count.

**The gate that fires is `resolveStaff`, and the body is `forbidden` — not `hasAssignedRole` and not
`forbidden_role`.**

> **Corrected against the build, and the behaviour is better than the criterion described.** A
> manufacturer partner never reaches the role check on this route, so **their refusal is
> byte-identical to an anonymous caller's**. They cannot learn that the console recognises their
> session at all — **X-AC-2's indistinguishability extends to the role**, which is strictly more
> than this criterion asked for.
>
> **The criterion named the second gate because the design named it.** The first one makes the
> second unreachable here. **Do not "harmonise" this to `forbidden_role` for consistency with other
> ops routes** — that would trade a real non-disclosure for a tidier table, on the one surface where
> the competitive leak is sharpest.
...
**X-AC-13 (the split path trusts no client field either)** — client-supplied fields outside the
route's accepted set are **ignored entirely**. **Mutation-proven at 3a.**
...
| **Quote lifecycle — post-issue** | Panel absent entirely, and the `why` URL refuses. **Not a failure state** — no retry. | D2, WHY-AC-11, WHY-AC-42 |
| **The rationale read fails** | Panel **stays**, says **it could not be read**, offers **retry**. **Never "not recorded".** | **WHY-AC-42** |
| **The rationale read is in flight** | Skeleton lines in the shape the panel is about to be. | **WHY-AC-42** |
| **A room-label, colour or dimension edit — including during an outage** | Figures carried forward verbatim, no catalogue read. | A4, SNAP-AC-16 |
| **A dimension or quantity edit on an AI-priced line** | Derivation writer: figures re-derive with the price and both snapshots. | A6 |
| **A customer changes only the glass** | Figures re-resolve **from the glass**; the row keeps its old id. **Nothing downstream reads that id as the answer.** | **D16** |
| **The panel's verdict on that same line** | **"A person chose this product."** The **glazing term** decides it. | **WHY-AC-29** |
| **A line a human changed** | Same three lines; **"Chosen"** carries the attribution and **"This one"** names *what* changed. | **D20**, WHY-AC-28 |
| **A composite parent (machine)** | "This one" **names the make-up**; no figures, and **no absence sentence** — nothing is missing. | WHY-AC-4, B4 |
| **An ops-created split** | **"These ones"** with per-unit figures; **"+N more units"** above three. | WHY-AC-37, B5 |
| **An opening with no thermal requirement** | "Had to meet" says so; **"Chosen" claims no thermal victory** — the cheapest that fitted. | WHY-AC-3, WHY-AC-5 |
| **A run from an earlier model** | **"This one" still shows the figures**; only the reasoning is unavailable. | WHY-AC-10, B8 |
| **A line saved before the capture existed** | Column `NULL` → **"not recorded"**, and **nothing is looked up**. | **D19**, §9.0 |
| **A line whose variant was nulled but whose product still stands** | Present-and-null meaning **"no selection was made"**. **The row settles it.** | §9.0, WHY-AC-9 |
| **A run stored with zero candidate rows** | **Not "reasoning not recorded".** A universal quantifier over the empty set is true and would send it to the wrong sentence. | WHY-AC-9, §12 |
| **An opening estimated more than once** | **The most recent run only.** | **D19** |
...
>
> > **Break the thing on purpose and watch the test go red.** If it cannot go red, the assertion was
> > decoration.
>
...
> **in production it is a branch nobody takes** — WHY-AC-9's second sentence would have been decided
> by a vacuous `every()` over a run with **zero candidate rows**. **Can this be true for a reason I
> did not intend?**

Not a test plan — seventeen places where the obvious test would pass a wrong implementation:

1. **WHY-AC-29's fixtures — two, and one of them is the whole point.** A product change is caught by
   any comparison. **A glazing-only override is caught only by comparing the pick.** *(Proved: the
   developer reverted to product+variant and it went red on that case alone.)*
...
4. **X-AC-1 must be executed for both callers separately.** **X-AC-13 must be an executed attempt.**
   **And X-AC-3 must assert the body it actually gets** — `forbidden`, not `forbidden_role`; a test
   written from the old criterion would fail against correct code.
5. **VIEW-AC-10, and the selector that became a trap.**
6. **WHY-AC-39/40/41 assert absences.** Enumerate and assert the *set* — **and the set is not empty
   in the failure state** (WHY-AC-42's retry).
...
10. **A JUSTIFICATION IS A CLAIM, AND IT MUST BE CHECKED — six times about the codebase, once about
    ...
    **THE FOURTH SHAPE — A MEASUREMENT CARRIED FROM ONE PATH TO ANOTHER.** *"A measurement is
    evidence for the thing measured. Generalising it to a sibling path is the same defect as an
    unexecuted claim, wearing a result"* — and harder to catch, because a number looks like
    verification.
...
13. **A CRITERION IS CHECKED AGAINST THE OTHER CRITERIA IT CONSTRAINS — five instances, none found by
    reading it alone.** VIEW-AC-1; SNAP-AC-1 vs SNAP-AC-9; SNAP-AC-1 vs SNAP-AC-14;
    WHY-AC-27/WHY-AC-4 vs SNAP-AC-8; **WHY-AC-29 vs §7.0 and D16.**

    **The trigger: when a phase establishes a new distinction, walk the criteria written before it.**
    **And the walk must enumerate EVERY distinction the phase established, not only the one that
    prompted it** — revision 28 walked §9 against SNAP-AC-8 and missed §7.0's *pick*, defined the
    same day. **A partial walk reads exactly like a complete one afterwards.**

14. **A CRITERION GENERALISED FROM ONE INSTANCE MUST NAME THE PROPERTY THAT MADE THAT INSTANCE WRONG,
    NOT THE BEHAVIOUR IT EXHIBITED.**
15. **ONE RULE IN TWO PLACES — the defect Phase 3a produced four times.** **The remedy is not
    vigilance; it is a single home.** **And when a tool is dropped for being noisy, its findings do
    not go with it.**
...
16. **PREFER A SHAPE THAT MAKES THE ERROR IMPOSSIBLE OVER A RULE THAT FORBIDS IT — a rule needs a
    ...
    D12's deletion of `ElevationLegend`. **When a criterion forbids a use, ask whether the thing
    being misused needs to be reachable at all.**

17. **A CRITERION, A TYPE AND A MOCK CAN EACH BE INTERNALLY CONSISTENT AND STILL DISAGREE — AND ONLY
    ONE OF THE THREE HAS THE OWNER'S SIGNATURE (new, revision 30).**

    This surface is described in **three** places: **§9's criteria**, **the design's DTO union**, and
    **`docs/mocks/ops2-why-this-product.html`** — the artifact that went through the UX mock gate and
    that **the owner approved**. Each was internally coherent. **They disagreed in six places**, and
    nobody had compared them:

    - **B8** — `unrecorded` shows "This one"; the DTO omitted `current`; §9 was silent.
    - **B5** — an ops split says **"These ones"**, lists per-unit figures, cuts at three; WHY-AC-37
      deferred to WHY-AC-8's **singular** shape.
    - **B3/B12** — "This one" carries *what* changed; WHY-AC-28 forbade it.
    - **B4** — a composite parent's "This one" names the make-up; WHY-AC-4 would have printed an
      absence.
    - **B11** — an absent requirement gets its own "Chosen" sentence; WHY-AC-5 had five tiers and no
      sixth.
    - **The failure state** — the mock ruled it and **§9 had no criterion at all**.

    **The precedence, and it is not "the spec wins":** where the mock shows a *state of the surface*,
    **the mock is the owner's approved artifact and the spec is the one that is wrong**. Where a
    **ruling** contradicts the mock, the ruling wins (this document's header, and R25 vs the legend
    is the worked example). **Rulings beat pictures; pictures beat unwritten criteria.**

    **The check, and it is one nobody had run:** *walk every state the mock draws and find the
    criterion that governs it.* A state with no criterion is not a state that will not ship — **the
    developer built the mock's behaviour and was right to** — it is a state **nobody will verify**,
    because the tester walks the criteria. **Five of the six above would have shipped correct and
    unverified.**
...
design's ambiguity rule.** **Two registers, one per document** — **and a mock that is neither**
(§12 note 17).
...
| 2 | **WHY-AC-10** — a pre-0055 run says the reasoning was not recorded | **DISCHARGED — owner (D19).** *(Revision 30 adds what it still shows: the figures.)* |
...
| 10 | **WHY-AC-28** — three labels kept | **DISCHARGED — owner (D20).** *(Revision 30: the qualifier on "This one" is the mock's, not a fourth line.)* |
| 11 | **§10** — the uniform refusal adopted as written | **RETIRED.** *(X-AC-3's correction describes which gate fires; it changes nothing.)* |
...
| 23 | *(WHY-AC-29's "product+variant" comparison after D16)* | **VETOED — architect's 3b design refresh.** A criterion **correct when written and made wrong by a later ruling**. **D16's second dependency** |
| 24 | *(that §9 described every state the approved mock draws)* | **VETOED — revision 30's walk.** Never a tagged assumption: **an unexamined belief that the criteria and the signed artifact agreed.** They disagreed in **six** places, and **§9 had no criterion at all for the read's failure state**. Registered so the next feature's walk is a step somebody owes rather than a habit somebody has |

---

## 14. Decisions needed

**One, and it is the owner's — the developer raised it rather than assuming, and it needs a criterion
before it is built either way.**

### Does the "Why this product" panel appear on the record's desk canvas?

**What he sees:** at desk width the project record shows the selected line in a canvas beside the
rail. That canvas renders the **same `LineReview`** the line page does — and **currently passes
`why={null}`**, so the panel does not appear there.

**Every state he approved shows the panel on the line page** (mock B1–B12, including **B10 "Desk —
the same panel, unchanged"**, which is the line page at desk width). **No approved state shows the
record canvas with a panel, and no criterion requires one.**

**Recommendation: yes, show it — but only with a criterion written first.** The reviewer's job is the
same in both places, and the canvas exists so a reviewer can read a line without leaving the record;
a panel that vanishes when they use the faster surface teaches them to distrust the faster surface.
**Phase 2 settled the analogous question the same way** — VIEW-AC-5's *"every enlargeable drawing"*
forced the canvas to open the viewer, and D11 then had to rule what its back control did.

**But it is not free, and these are the parts that need his ruling rather than mine:**

- **The detail screen's route is `/projects/:id/line/:lineId/why`** — a **line** address. From the
  canvas, opening it would push the reviewer onto a **line** URL, which is the journey D11 had to
  rule for the drawing viewer. **The same question returns: where does back go?**
- **The canvas shows one line at a time**, so the panel would re-fetch per selection. That is a
  read-per-selection on a surface built for glancing down a list.

**The alternative worth naming:** leave it off the canvas, and let the panel be a reason to open the
line — the record stays a list, the line page stays the place you audit.

**What a veto costs today:** nothing built either way. **What it costs later:** if this is added
after 3b ships, it is a second journey with its own back-destination ruling, exactly as the canvas
enlargement was in Phase 2 — **added at a review stage, with no criterion, and found by the tester.**

### Also outstanding, not part of this question

- **§13.1 (VIEW-AC-9)** — OPEN and already shipped; a veto costs rework.
- **A5** — the architect's, **vetoable as one pair with the design's ambiguity rule** at acceptance.
- **D16's condition is now three criteria wide** (WHY-AC-4, WHY-AC-29, the DTO omission) — worth
  restating at acceptance, because its cost is no longer "one label".
- **The deploy** — `0058` is local-only and reaches production **with 3b** (D17), under the deploy
  protocol including a preview smoke-test, because it touches a save path.
