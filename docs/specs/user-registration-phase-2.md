...
Status: **revision 5 — owner copy ruling ("better prices" retired; five approved strings) folded in.
§12 "Decisions needed" is EMPTY.**
...
**Copy authority:** §7.2 holds the owner-approved customer-facing strings and §7.1 the email
fallbacks. Where this spec and any mock, interaction spec or implementation disagree on a
customer-facing string, **this spec wins** and the correction is routed rather than argued.
...
| 4 | **(a) Owner ruling P2-D5 — "builder vs tradie: no difference".** The question is **removed from the product**: nothing captures, stores, displays or returns the label; `user.trade_label` is dropped from migration 0054 (not yet applied to production, so removed rather than reversed). The business group is exactly **ABN + business name**, identical at all three doors. Amended: §2.1.10, §4.1–4.4, §7, §9, §10, AC-P2-1, AC-P2-3, AC-P2-10, **AC-P2-15** (restated as a phase-wide absence assertion), AC-P2-36, AC-P2-52, AC-P2-64, `P2-A2`. **(b) One universal registration flow** — Phase 1's, with the optional business group inside its details step; `/trade-account` is a marketing page hosting that same signup, differing only in that the group renders already revealed (§8.10) |
| **5** | **Owner copy ruling — "better prices" is retired.** *Trade pricing is the NAME of the thing; the possessive line does the explaining.* It is a **status, not a deduction** — consistent with the discount being folded into line prices with no discount line anywhere. "Better prices" is comparative with no reference point and reads as a markdown; "trade pricing" is the term the audience uses, and the brand is AMJ Trade **Direct**, so trade is already the promise. Five owner-approved strings recorded verbatim in **§7.2**; the `trade_approved` fallback in §7.1 replaced verbatim; the other three fallbacks swept for the same register; string-quoting criteria updated (**AC-P2-9**, **AC-P2-18**, **AC-P2-48**); **AC-P2-64 gained a comparative-language clause** (see §7.3 for the bounded phrase list and why it is a phrase list rather than a ban on the word "better") |
...
- A tradie who has never used the site can arrive cold at `/trade-account`, prove their business,
  and **browse and configure at trade pricing from that moment** (P2-D1 — the primary path).
...
- **Trade pricing is described as a status, never as a saving** (rev 5 copy ruling).
...
1. **Three entry points into ONE verification, inside ONE registration flow** (P2-D1/D2/D3, plus the
   ...
   - **(b) The profile/account page** — a first-class ABN affordance (the approved card string is in
     §7.2).
...
6. **Four outcome emails** — acknowledgement, approval, rejection, revocation — each a
   **Sanity-authored template with a Worker fallback** (§7.1), none of which states a percentage, a
   turnaround time, or a comparative price claim.
...
9. **Advertising the existence of trade pricing (D5)** on four named surfaces, in the approved
   language of §7.2 — never the percentage, never a comparative claim. Phase 1's AC-41 silence ends
   here, by design (§5.8).
...
- **Any distinction between builders and tradies** (P2-D5). If a later feature — plans upload,
  ...
  matters, and it justifies the question by what it changes.
...
|---|---|---|
| Private customer | Untouched everywhere, except that they now learn trade pricing exists — and that *they* may qualify if they hold an ABN (§7.2, deliberately "may qualify", never a promise before verification); a rejected application costs them nothing (A3) | — |
| Tradie | Cold arrival at `/trade-account` → verified → **trade pricing while configuring**, not after; a gmail address costs a wait and a human, never a rejection; the ABN is asked once and lives in one place; **two fields, not three** | The quote-level representation of trade pricing (#10) |
...
| Ops | A queue holding only judgment calls, each with the ABR evidence gathered and duplicates flagged; **any assigned-role staffer can clear it**; ABN + trade status on every customer record; revocation; a decision history; **existing staff rows pinned to 0**; the customer's phone and address finally visible on the project record | A per-account rate editor (still D1 by hand); a trade signal on the quote itself (#10) |
...
The page keeps its marketing content — **in the approved language of §7.2** — and **loses its fake
form**. In its place it hosts **the same registration flow** every other door uses: email → OTP (the
Phase-1 `OtpSignIn` component, Turnstile and rate limits unchanged) → details, with the **business
group already revealed** because a visitor who arrived here came for exactly that. That reveal is the
*only* entry-point-specific behaviour in the phase.

- Auto-pass → the page says the trade account is **active**, and the next price the visitor sees
  anywhere on the site is a trade price. No percentage, no saving, no "you saved" line.
...
verification affordance — the approved account-card string is in §7.2, and its "**may** qualify" is
deliberate: the reader may be a private customer with an ABN who does not know they qualify, and
nothing may promise an outcome before verification. Same two fields, same verification, same two
outcomes, plus the status of any application in flight and the history of past ones.
...
same **optional** business group, collapsed until wanted, introduced by the approved gate helper
string (§7.2) — which states in its own words that it will not hold up the submission. The Phase-1
gate contract holds without exception:

- An empty ABN never disables Submit and **never appears in the "Still needed:" caption**.
- Entering an ABN makes business name required alongside it — the same rule as every other door.
...
- An account already verified, or with an application pending, sees no business group.
...
that failed** (P2-A3). That is both a courtesy and a security property (AB-P2-7): the endpoint must
not become a tool for testing ABNs or probing business names.
...
| `POST /api/auth/profile` / the referral payout form (existing, D6, unchanged by owner ruling Q7) | Writes the ABN for **payout** purposes; **grants nothing** | **Refused** while the account is trade-verified or has an application pending (P2-A4, AB-P2-12) |
...
- **AC-P2-9** *(amended, rev 5 — copy ruling)* — Given a signed-in account with no ABN, When it
  opens the profile/account page, Then a first-class card carries the **approved account-card
  string** (§7.2) — which names trade pricing as a thing they **may qualify** for, says the ABN will
  be checked against the Australian Business Register, and contains **no percentage, no saving, and
  no comparative price claim**; and Then it promises **no outcome** before verification.
...
- **AC-P2-14** — Given a signed-in customer with no ABN at the gate's details stage, When the panel
  renders, Then the **optional** business group is available under the approved gate helper string
  (§7.2), and leaving it empty neither disables Submit nor adds anything to the "Still needed:"
  caption.
...
- **AC-P2-17** — Given a well-formed ABN in the gate's business group, When the customer submits,
  ...
  made slow).
- **AC-P2-18** *(amended, rev 5 — copy ruling)* — Given a submission that carried an ABN, When the
  confirmation is shown, Then it acknowledges that the ABN is being checked, states **no
  percentage**, **no timeframe** and **no comparative price claim**, promises **no outcome** before
  verification, and **does not promise that this quote will be repriced** (P2-D4).
...
### 5.7 Outcome emails (see §7 for the templates and the approved copy)

- **AC-P2-43** — Given an application is approved (auto or by ops), When the decision lands, Then
  the applicant receives the `trade_approved` email whose fallback copy is **the owner-approved
  string of §7.2 verbatim**, containing **no percentage** and no figure from which one could be
  derived.
...
- **AC-P2-63** — Given an `emailTemplate` document authored in Sanity for one of these keys, When
  ...
  cached).
- **AC-P2-64** *(amended, rev 4 and rev 5)* — Given all four Phase-2 emails in both their authored
  and fallback forms, When their rendered subject, body and **variable set** are inspected, Then
  none contains:
  (i) a discount percentage or a pair of figures from which one could be derived;
  (ii) a promised turnaround time;
  (iii) any builder/tradie variable or wording (P2-D5);
  (iv) **any phrase from the retired-comparative list of §7.3** — trade pricing is named as a
  status, never sold as a saving (rev 5 copy ruling).
...
- **AC-P2-47** — Given **every** customer-facing surface in the product, When it renders for any
  ...
  AC-36).
- **AC-P2-48** *(amended, rev 5 — copy ruling)* — Given this phase, When the site is swept for copy
  about trade pricing existing, Then it appears on **exactly these four surfaces and nowhere else**:
  `/trade-account`, the profile affordance, the submit gate's business-group helper text, and the
  outcome emails; the home page, the nav, the quote builder and the review screen's pricing panels
  stay silent (Phase 3 owns wider marketing). And When those four surfaces are searched for **any
  phrase in the retired-comparative list of §7.3**, Then none appears — the copy names trade pricing
  as a status and lets the possessive line do the explaining.
...
- **AC-P2-51** — Given the migration, When it is applied, Then **exactly the three email addresses
  ...
  rate; the two without an ABN are verified with **no ABN written**; and **no other customer row's
  `discount_percent` or trade status changes**. Matching is by explicit address — never "every
  customer row that exists when the migration runs".
- **AC-P2-52** *(amended, rev 4)* — Given the migration, When it is reviewed and applied, Then it
  ...
  table rebuild** — it is numbered after `0053`, and **it defines no builder/tradie column**:
  `user.trade_label` is removed from migration 0054 outright (0054 has **not** been applied to
  production, so this is a removal, never a reversal).
...
- **AC-P2-55** — Given a project in the ops console, When staff open its record
  ...
  and full account address are shown read-only (Phase 1 seam §6.6).
...
- **AC-P2-59** — Given the existing regression suites, When this phase is complete, Then
  `scripts/tests/api.test.mjs`, `scripts/tests/web/customer.spec.ts`,
  `scripts/tests/web/registration.spec.ts`, `scripts/tests/web/registration-gate-caption.spec.ts`,
  `scripts/tests/email-templates.test.mjs` and the referral suites all pass — with any edit to them
  **declared** in the developer's report and justified by AC-P2-49, the new business group or the
  new template keys, never silent.
...
- **AB-P2-5 — unauthenticated access.** Given no session, When every endpoint added in this phase is
  called, Then each returns 401/403 with no data, and **the ABR stub records zero calls**.
...
- **AB-P2-7 — the criterion oracle.** Given applications that fail on the ABN status, on the name
  ...
  byte for byte, Then they are identical.
...
- **AB-P2-9 — the borrowed ABN.** Given a valid, active ABN belonging to a real business the
  ...
  an applicant with a matching domain and a copied business name can auto-pass. Bounded, not
...
- **AB-P2-11 — staff and partner roles.** Given a `type='internal'` account, When it applies for
  ...
  Then it receives 403 (assigned roles only, owner ruling Q4).
- **AB-P2-12 — swapping a verified ABN.** Given a trade-verified account, When it writes a different
  ABN through `POST /api/auth/profile`, Then the write is refused with a specific error and the
  stored ABN is unchanged.
...
- **AB-P2-15 — revocation takes effect immediately.** Given a customer with a live session and an
  ...
  retail prices — the grant is read from the account row per request, never baked into a session.
- **AB-P2-16 — the ABN never travels through one of OUR URLs.** *(amended, rev 3)*
  **(a) Executable clause — our own surfaces.** Given every request this phase adds **that a browser
  ...
  supplied as a query parameter instead, Then it is ignored or refused.
  **(b) Carve-out — the outbound registrar call is out of scope for this criterion.** The ATO ABN
  ...
  string of the server-to-server request **we make to the registrar**. That leg is a TLS call from
  the Worker to the ATO, is never logged by us (AB-P2-8), never reaches a browser, and cannot appear
  in a `Referer` header — the tester records it as designed-and-accepted, **not as a finding**.

---

## 7. Customer-facing copy

### 7.1 The four emails — Sanity templates with Worker fallbacks
...
| Key | Fires when | Vars | Fallback subject | Fallback body |
|---|---|---|---|---|
| `trade_ack` | An application is created and queues for review | `name`, `business` | We're checking your trade account details | "Thanks — we've got the ABN and business details for {business}. We're checking them against the Australian Business Register and we'll be in touch. Your account works as normal in the meantime." **No timeframe.** |
| `trade_approved` | An application is approved — auto or by ops | `name`, `business` | Your trade account is active | **Owner-approved, verbatim:** "Your trade account is active. Trade pricing applies from now on — when you're signed in, the prices you see are already your prices." *(PM addition, removable: "Anything already with us for review will be priced by our team." — it exists to stop a customer expecting the quote they already sent to change on its own, per P2-D4.)* |
...
| `trade_revoked` | Ops revokes trade status | `name` | A change to your trade account | "Trade pricing no longer applies to your account. From now on you'll see our standard prices. If you think that's a mistake, reply to this email and we'll sort it out." **No percentage, no blame, no comparison.** |

**The variable set is exactly `name` and `business`** — no `label`, `tier` or `accountType`
variable, and no template may introduce one (AC-P2-64).
...
### 7.2 Owner-approved on-site strings (verbatim — rev 5)

Recorded here so the spec, the mock, the interaction spec and the Worker cannot drift. **These are
the owner's words**; the ux/ui stage may set them, not rewrite them.

| Surface | String |
|---|---|
| Trade page hero | "Trade accounts get trade pricing, priority review, saved details, and a name to call." |
| Trade page benefit | "Trade pricing, everywhere" / "It applies while you configure — not just on the quote we send back." |
| Account card (to a private user) | "Have an ABN? You may qualify for trade pricing. Add it and we'll check it against the Australian Business Register." |
| Submit gate helper | "Got an ABN? Add it and we'll check whether you qualify for trade pricing. It won't hold up this submission." |
| Email `trade_approved` | "Your trade account is active." / "Trade pricing applies from now on — when you're signed in, the prices you see are already your prices." |

**"You may qualify" is deliberate** on the account card and the gate helper: the reader may be a
private customer who holds an ABN and does not know they qualify, and **nothing may promise an
outcome before verification** (AC-P2-9, AC-P2-18).

### 7.3 The retired comparative — what the ban actually is (rev 5)

The ruling: **"trade pricing" is the NAME of the thing; the possessive line does the explaining.**
It is a *status*, not a deduction — which is exactly consistent with the discount being folded into
line prices with no discount line anywhere (CONTEXT.md, never-disclose-by-subtraction). "Better
prices" is comparative with no reference point and reads as a markdown; the brand is AMJ Trade
**Direct**, so trade is already the promise.

**Enforced as a bounded phrase list, not a ban on a word.** A blanket "no comparative language"
assertion would be unexecutable, and a ban on the bare word *better* would be brittle (it would trip
"better for big jobs" and other legitimate copy). So AC-P2-64(iv) and AC-P2-48 assert the absence of
these **price-adjacent phrases**, case-insensitively, on the four advertising surfaces and in all
four emails:

`better price` · `better pricing` · `better prices` · `cheaper` · `discount` · `% off` ·
`save $` · `you save` · `RRP` · `was/now` (a struck-through price pair)

If a legitimate future string trips the list, the list is edited **with the owner's ruling cited** —
not quietly bypassed in a test.
...
5. **The ABR client must be stubbable.** Tests must never touch the live ABR: an injectable
   base URL/fetch seam is mandatory.
6. **Migrations are append-only, numbered after `0053`, additive only, never a table rebuild**; the
   `d1-migration-safety` skill is mandatory reading, and a production export precedes any remote
   apply. **Migration 0054 defines no builder/tradie column** (P2-D5).
...
8. **Inbound ABNs arrive in POST bodies; the outbound ABR GET is the one exception and is never
   logged.** That URL must never be logged, echoed in an error message, returned to a client, or
   reconstructed on any customer or ops surface (AB-P2-8, AB-P2-16).
...
11. **Customer-facing strings come from §7.** The five approved strings are used verbatim; the
    retired-comparative list of §7.3 is enforced by test, not by memory.
...
2. **An application record per attempt** — the frozen submitted ABN and business name, the ABR
   ...
   for a human decision. History, not a second home for the ABN (§4.7).
...
4. **CONTEXT.md** gains *Trade account / trade-ness* and *Trade application* (architect owns the
   ...
   thing to this system** (P2-D5) and that **trade pricing is a status, never described as a
   saving** (rev 5).
...
4. Profile affordance: the **approved account-card string** is present verbatim; a private account
   adds an ABN and reaches the same two outcomes; a verified account's profile shows status with no
   editable ABN field (AC-P2-9/10/11).
5. Submit gate: the **approved helper string** is present verbatim; business group empty → submits in
   one press and never appears in the caption; ABN malformed → Submit disabled and named in the
   caption; ABN valid → submits with no ABR call in the request path and shows the acknowledgement
   (AC-P2-14/16/17/18).
6. A verified account at the gate sees no business group (AC-P2-19).
7. **The percentage-and-comparative sweep** across all four advertising surfaces: no `%`-bearing
   discount copy, no subtraction pair, and **no phrase from the §7.3 retired list**
   (AC-P2-47/48).
8. **No ABN in any browser-made URL** across the whole flow (AB-P2-16a).
9. **The builder/tradie absence sweep** — no control or copy offering the choice on any customer
   surface (AC-P2-15).

**Playwright — ops queue coverage in `scripts/tests/web/ops.spec.ts` (existing harness reused):**
...
10. A queued application appears with its reasons and ABR snapshot; approve **as a non-admin
    ...
    on the row, the detail, or the customer 360.
...
- Grant and revoke arithmetic (AC-P2-28/29/30/34, AB-P2-14).
- Authorization matrix: anonymous, customer, cross-customer, manufacturer staff, **non-admin
  assigned-role staff (must succeed)**, admin (AB-P2-1…5, AB-P2-11, AC-P2-37).
- Rate limits, mass assignment, oversized input, ABN-swap refusal (AB-P2-6, AB-P2-1, AB-P2-13,
  AB-P2-12).
- **Credential and ABN containment:** a forced ABR failure produces an error response and log output
  containing neither `ABR_GUID` nor the submitted ABN nor the outbound URL; an ABN supplied as a
  query parameter is ignored or refused (AB-P2-8, AB-P2-16a).
- **Label absence at the data layer:** a `trade_label` / `tier` / `accountType` key posted to the
  application endpoint is ignored; no DTO or ops payload carries one; no such column exists
  (AC-P2-15).
- **The four emails:** right dot-free `templateKey` per event; with no Sanity document the fallback
  sends; with a stubbed authored template the authored copy wins; **the `trade_approved` fallback
  matches §7.2 verbatim**; and no rendered email — authored or fallback — contains a percentage, a
  timeframe, a builder/tradie variable, or a §7.3 phrase (AC-P2-43…46, AC-P2-61…64), extending
  `scripts/tests/email-templates.test.mjs` where that suite owns the mechanism.
...
**Unit — `scripts/tests/unit.test.mjs` (existing):** name-matching and email-domain-plausibility as
pure functions, with a table of AU business-name shapes and a free-mailbox list (AC-P2-22/23,
E-P2-20).
...
the per-recipient OTP cap is 5 per 15 minutes.
...
1. **Verification core + door (a).** ABR client with its stub seam, the triple, the duplicate rule,
   ...
   component, `/trade-account` rebuilt around the universal flow with the §7.2 hero and benefit
   strings. Deployable on its own.
2. **Ops queue + the four emails + revocation.** The queue surface, decisions by assigned-role
   ...
   record.
3. **Doors (b) and (c) + the migration + the two carried seams.** Profile affordance (§7.2 card
   string), the gate's business group (§7.2 helper string), staff pinning and grandfathering, the
   payout-form prefill, the ops contact line.

---

## 12. Decisions needed (owner)

**EMPTY.** Every question raised at revision 1 has been answered, and revisions 3, 4 and 5
introduced none:
...
| Q3 | Grandfathering — which accounts, identified how? | **Three named addresses at deploy time** (§5.9); two hold no ABN and are granted by owner decision |
| Q4 | Who may approve / reject / revoke? | **Any assigned-role staff member**, not admin-only; logged; manufacturer accounts excluded |
| Q5 | Builder/tradie label at the submit gate? | **No** — and **superseded by P2-D5**, which removes the question from the product entirely |
| Q6 | Keep the two owner add-ons? | **Keep both** → §2.1.11, AC-P2-54/55 |
| Q7 | Private accounts still payable via the referral payout form? | **Leave it** — D6 unchanged → §4.7 |

Revision 3 was a **correction**; revisions 4 and 5 are **owner rulings handed down** — a scope
reduction (P2-D5), a structural correction (one flow), and a copy ruling ("better prices" retired).
None was a question back.

The only owner action item remaining is not a decision: **the four `emailTemplate` documents need
authoring in Sanity Studio before or at deploy** (§7.1). A missing document is safe — the Worker
...
1. **"ABN keeps ONE home" (P2-D2) vs. ops history (A3) and the duplicate rule (D2.1).** Resolved in
   §4.7: the account row is the live fact; the application record is a frozen historical copy.
2. **Two writers of `user.abn`.** Resolved in §4.7 — the payout path grants nothing and is refused
   while verified/pending (P2-A4); **affirmed by the owner at Q7**.
3. **Two `5`s.** D4's business-account default lives in code; migration 0032's column `DEFAULT 5`
   stays (a rebuild is forbidden). The code constant is the authority.
4. **P2-D3 ends Phase 1's AC-41 silence.** AC-P2-49 supersedes AC-41 on four named surfaces, openly.
5. **"Never a hard block" (D2.1) is a knowingly accepted risk.** With AB-P2-9, ABN *ownership* is
   never proven by this system; the controls are visibility, human order review, and revocation.
6. **"ABR benefits payout validation for free" vs D6's "no referral code changes".** Payability
   stays on the checksum-only test.
7. **The Phase-2 scope list omitted the two owner add-ons.** Raised at the gate, **confirmed in
   scope** — not a descope.
8. **Grandfathering is not verification.** Two of the three have no ABN; provenance is labelled
...
9. **My own AB-P2-16 was unsatisfiable** (rev 1–2): the ATO service is GET-only. Caught by the
   architect, amended in rev 3 — a security criterion must name the *boundary* it governs, not just
   the forbidden shape.
10. **D7's builder/tradie label vs P2-D5** (rev 4). The *actor* distinction survives as vocabulary;
    the stored property does not. The verbatim actor text is preserved with a supersession note
    rather than edited, so the record stays honest (§3).
11. **"Better pricing" in the grill vs the rev-5 copy ruling** *(new)*. The grill's reframe says what
    stays exclusive to verified businesses is "**better pricing** — shown as lower prices, never as a
    visible discount line". That was the *economic* description, and it remains true. It is **not**
    customer-facing copy: the owner has now ruled that the words customers read name the thing
    ("trade pricing") rather than compare it. Both statements stand at their own altitude — §7.3
    records the distinction so nobody "restores" the grill's phrasing into a UI string.
...
|---|---|---|---|
| `ASSUMED: P2-A1` | An ABN entered at the submit gate must not block or delay submission; my refinement — a **malformed** ABN does block Submit and is named in the caption | §4.4, AC-P2-16/17 | assumed, unvetoed |
| ~~`ASSUMED: P2-A2`~~ | No builder/tradie label at the gate | §4.4, AC-P2-15 | **DECIDED — Q5**, then **SUPERSEDED — P2-D5** (removed product-wide) |
...
| `ASSUMED: P2-A4` | `user.abn` cannot be written through the profile/payout path while verified or pending | §4.7, AB-P2-12, E-P2-19 | assumed; **affirmed at Q7** |
...
| `ASSUMED: P2-A7` | Trade status reaches the client as a status only; no endpoint serves `discount_percent` to a customer; revocation bites on the next request | AC-P2-47, AB-P2-15 | assumed, unvetoed |
| ~~`ASSUMED: P2-A8`~~ | An acknowledgement email is sent when an application queues | AC-P2-45, §7.1 | **DECIDED — Q2** |
| `ASSUMED: P2-A9` | The ops queue lives inside the existing Customers area with a pending count on the console landing surface | AC-P2-35 | assumed, unvetoed |
| `ASSUMED: P2-A10` | One pending application per account; re-submitting while pending is refused with the current status | §4.4, AC-P2-12, E-P2-5 | assumed, unvetoed |
...
| `ASSUMED: P2-A12` *(narrowed, rev 5)* | Fallback wording for `trade_ack`, `trade_rejected` and `trade_revoked`, and the one PM-authored sentence in `trade_approved`, are mine and rewordable within the §7.3 constraints. **The five §7.2 strings and the `trade_approved` sentences are owner-approved verbatim and are NOT assumptions** | §7.1, §7.2 | assumed only where marked |

Grill assumptions **A1, A2, A3, A4** remain live and are honoured by this spec.
...
|---|---|---|
| E-P2-1 | ABN typed with spaces or whitespace | Normalised to 11 digits before validation, storage, duplicate comparison and display |
| E-P2-2 | ABR reachable but slow | Bounded by a timeout; on expiry the application queues (`abr_unavailable`). No user-facing request hangs on ABR |
| E-P2-3 | ABR returns several trading names | Matched against the entity name **and every** trading/business name; any match satisfies criterion 2 |
| E-P2-4 | Entity active but the ABN is a branch/GST-only variation | Treated as active; GST registration is **not** a criterion |
| E-P2-5 | Applicant applies twice in a row | One pending application per account; the second attempt is refused with the current status (P2-A10) |
| E-P2-6 | Approved account applies again with a **different** ABN | New application; trade status and pricing **retained** while pending; if rejected, ops decides whether to revoke (never automatic, P2-A11) |
| E-P2-7 | Rejected applicant re-applies with corrected details | Allowed; new record; the earlier decision stays in history (A3) |
...
| E-P2-9 | Grandfathered account has no ABN on file (two of the three) | Trade-verified, provenance `grandfathered`; ops record says "no ABN on file"; nothing invents a value |
| E-P2-10 | Approved entity later lapses on ABR | Not detected in this phase (P2-A6). Ops revokes when it surfaces — and the customer is emailed (Q1) |
| E-P2-11 | Customer's sign-in email changed by ops after approval | No re-verification, no status change; the domain criterion was evaluated at decision time and is in the snapshot |
| E-P2-12 | Trade-verified account with GST mode `ex` | Mode governs display, trade status governs price. The two never interact |
| E-P2-13 | `/trade-account` visitor hits the OTP rate limit (5 per address / 15 min) | Phase-1 E2 copy applies; the entered ABN/business values survive the failure |
| E-P2-14 | Customer abandons `/trade-account` after the OTP, before the ABN | A working private account exists (Phase-1 `NameStep` rules); no application; door (b) picks them up |
| E-P2-15 | Verification succeeds mid-configuration | Live previews price at the new rate on the next preview; **no stored line is rewritten** (AC-P2-31) |
| E-P2-16 | Quote submitted with an ABN, approved before ops reviews it | Ops reviews and prices at the current rate — P2-D4's intended path |
| E-P2-17 | Two staff decide the same application | First wins; the second gets 409 (AC-P2-39) |
| E-P2-18 | ABR GUID missing in a preview/dev environment | Everything queues (`abr_unavailable`); one startup warning; no crash, no fallback to "approve" |
| E-P2-19 | Pending applicant opens the referral payout form | ABN pre-filled but the write is refused while pending (P2-A4); the form explains the ABN is being checked |
| E-P2-20 | Trading style vs ABR legal name ("Smith Bros" / "SMITH BROTHERS PTY LTD") | Fuzzy matching should pass it; where it does not, the outcome is a queue entry, never a rejection |
| E-P2-21 | Sanity down when an outcome email fires | The fallback sends (AC-P2-46); the failure is not cached, so the next send retries |
| E-P2-22 | A later feature genuinely needs builder vs tradie | It asks then, where the answer changes something — this phase leaves no column and no half-collected data (P2-D5) |
| E-P2-23 | A copy string legitimately needs a word on the §7.3 list | The list is edited with the owner's ruling cited, never bypassed in a test (§7.3) |
