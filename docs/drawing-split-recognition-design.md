...
**Guard-rail to ship with Stage 1:** route computed orientation into `advisoryRequirements` rather than the hard filter — the scaffold comment at `pipeline.ts:700-702` says it should already be there and it is not. Making orientation *more* available while it hard-filters would amplify any error.
