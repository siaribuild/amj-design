...
Status: **revision 4 — as built.** Revision 3 (D18 folded in; program status collapsed to On/Off) is the design of record; **§18 is the as-built register and governs wherever it and an earlier section disagree** — it was written at the design-conformance review of the implemented feature (2026-08-17) and corrects §6.1, §6.2, §7.5, §9, §10.2, §10.3, §10A.3 and §13.1 against what shipped, with the reason for each.
Author: architect
Date: 2026-08-15
...
- **`robots.txt` deliberately does NOT disallow `/r/`.** A disallow prevents *crawling*, so the
  noindex would never be fetched — and a widely-linked-but-uncrawlable URL can still be indexed
  without content ("indexed, though blocked by robots.txt"), the precise failure being prevented.
  One control, crawlable so it is seen.
- `/r/*` never appears in `/sitemap.xml`. True by construction (`buildSitemap` builds from
...
### 12A.1 `/r/<CODE>` — directives specified in §6.1
...
- The article **extends** the landing FAQ — it does not restate its questions or answers. The two
  pages target different queries; a restated article competes with its own landing page and both do
  worse.
- **No canonical between them.** `canonicalUrl` (`src/data/seo.ts:95`) is for duplicate
  documents; these are deliberately different documents, and cross-canonicalising would ask a search
  engine to discard one. Each page stays canonical to itself (the default). If a copy pass finds the
  article merely restating the landing content, the fix is the content, not canonical plumbing.
- The two pages interlink ("the short version" ↔ "full details, rules and terms") — which is also
  the answer-engine-correct shape: one authoritative long-form source, one conversion surface.
- If FAQ structured data is ever added, it is added to exactly one of the two pages — the one
  carrying that specific Q&A.
...
## 18. As-built register (design-conformance review, 2026-08-17)

Written after T0–T8 shipped, reviewing the implementation against this document. Each entry names the
section it corrects and why the divergence was accepted or refused. **Where this section and an
earlier one disagree, this one governs.**

### 18.1 Accepted divergences — the design was wrong, the code is right

| # | Section corrected | As built | Why the code wins |
|---|---|---|---|
| A1 | **§10.3** — `?from=&to=` was specified on `payouts/export.csv` | The range lives on `GET /payouts/history`; `export.csv` takes no parameters | The CSV **is** the ready queue — right-now unpaid confirmed money, with no date axis to filter on. A date filter there would emit a *partial transfer file*: some of what is owed paid, the rest silently absent. `paid_at` on the history rows is the real date axis, and a quarter or BAS range is exactly what the accountant asks of it. The parameters were specified against the wrong one of the two endpoints. |
| A2 | **§10.3** — `payouts/history` built during T8 | Built | It was already in §10.3, and T8's brief (the payout run) cannot be met without it. AC-44's reversal needs a payout id, and before this endpoint the only ids in existence were those a `mark-paid` response handed back — so the reversal button died on navigation, while a bounce arrives days later and reaches whoever is at the desk. In-design, not invented. |
| A3 | **§10.3 `ReferrerGroup`** | Groups carry `email` and `name` alongside `abn/bsb/accountNumber/accountName`, `amount`, `earningIds`, `referralRefs`, `oldestConfirmedAt`, `daysWaiting`, `overPromise`, `forcedByLongStop?` | The run emails each referrer at mark-paid (§9, AC-43), so the group — the unit of the run — must carry the address. It is personal PII on a staff-only surface that already shows customer emails throughout; no *banking* travels anywhere new, so §10A.1 is unchanged. `payoutPaidNotification` still hands the email template only `{name, amount, reference, date}`: the digits never leave the group. |
| A4 | **`worker/index.ts` asset branch** (not previously in §4) | `isAsset` now excludes any path starting `/api/` | Correct fix, correct place. Any `/api/` route whose last segment carries an extension was looked up in the asset bundle and 404'd before reaching its handler — a trap armed for every future route of that shape, not only `payouts/export.csv`. Renaming the route would have left the trap armed. The change cannot affect non-API paths, and an unmatched `/api/` path now gets Hono's 404 rather than the assets 404, which is the more correct answer. Guarded by the "the CSV is staff-only, and taking it is recorded" subtest, which fetches the real URL. |
| A5 | **§9 / §10.2 — clearing via `PUT` with blanked fields** | `DELETE /api/account/payout-details` (`leaveProgram`) | The PUT validates hard (BSB 6 digits, account 5–9 digits, non-empty name), so blanking through it is unreachable by construction — and should be: *leaving the program* is a different intent from *correcting a typo*, and the refusal ("$X is confirmed and hasn't gone out yet") belongs to one of them only. Membership is having payout details, so leaving is clearing them; no second membership record was invented. |
| A6 | **§6.2 — hook inside `findOrCreateUser`** | `findOrCreateUser` returns `{ user, created }`; `worker/routes/auth.ts` reads the `of_ref` cookie, calls `recordReferral` when `created`, and clears the cookie | A better seam. Cookie parsing and cookie clearing are boundary concerns, and keeping the call beside them leaves `findOrCreateUser` doing one thing; AC-7 still holds structurally, because the existing-user branch reports `created: false`. **The try/catch this section required is still required** — see F1. |
| A7 | **§4 — `referral-discount.ts` holds one function** | It holds three: `referralDiscountState`, `issuedReferralBadge`, `opsReferralReview` | All three are derived, read-only referral queries with no `lib/` dependency, so the leaf property (ADR-1) is intact and measurable: the module still imports `../types` and nothing else. Putting the badge there is what lets `orderDto` render it without `orders.ts` importing `referrals.ts` — ADR-1's "orders.ts is untouched by referral imports" is amended to "untouched by `referrals.ts`; importing the leaf is safe by definition". |
| A8 | **§10.3 `mark-paid {userIds, reference, includeUnderThreshold?}`** | `{userIds, reference, note?}`; naming an accruing referrer pays them | The override AC-61 asked for, with no flag for it. The request names *people*, never earning ids, so the server derives the earning set from its own queue — strictly stronger than §10A.3's id filter, because a tampered body cannot name an earning at all. |
| A9 | **§10.3 — `min_payout_balance > 0` requires `body.acknowledgeHold`** | Not built; the stated warning renders beside the field on the Program screen | ADR-8e's two structural measures — the warning at the point of decision and the 11-month long-stop force-promotion — are both built. The body flag was belt-and-braces on a staff-only endpoint whose only client renders the warning. **§13.7 scenario 6's second clause is withdrawn**; its first clause (long-stop promotion) stands and is tested. |
| A10 | **§7.5 placements row** ("shown while Off") | All four placements render nothing while Off (UX spec §4) | The ux-designer's argument is the better one and was approved at the mock gate: a placement pitching a rate in the present tense while nobody can join is the ACL s 18 / s 32(1) exposure, and a notice on the landing page cannot repair a pitch the visitor read somewhere else. The §17 recommendation to the product-manager ("placements stay while Off") is withdrawn. |
| A11 | **§9 — the payout history rows** | `payoutHistory` masks the frozen account number and writes no access-log row | Correct: it reads the *frozen* copies (where money went), never the live `user` columns (where money would go next), so it discloses nothing the log exists to record — and a list of everyone ever paid is the last place to put unmasked account numbers on one screen. §10A.1's "frozen copies … staff payout screen + CSV only" narrows accordingly: the history screen sees them masked. |
| A12 | **§10.3** — no ops linking action was specified | `POST /api/ops/referrals/link` (staff attach a code to an account; UX §8.4, A19) | Legitimate and correctly built: it calls `recordReferral`, so every gate the customer paths run applies identically — a privileged path that inserted directly would become the way around all of them. Added to §10.3 and to the table below. Its refusals are deliberately *specific* where the customer-facing ones are vague: vagueness exists so a stranger cannot probe which codes are real, and there is no stranger on an internal screen. |

### 18.2 §10A.3 authorization table — as built

Replaces the corresponding rows. Everything not listed is unchanged and was re-verified.

| Route | Who | The scoping filter, as built |
|---|---|---|
| `GET /r/<CODE>` | Anonymous | `SELECT 1 FROM user WHERE referral_code = ?` — existence only. **The payability, staff and program-On checks moved to recording time and are not repeated here** (accepted): the cookie carries only a code, `recordReferral` re-validates every gate at the moment attribution would actually happen, and *not* consulting payability here removes a side channel — under the designed version a stranger holding a real code could watch the `Set-Cookie` disappear and learn that its owner had cleared their bank details. One behavioural consequence, accepted: a click while Off followed by a signup after the program returns *is* attributed, which reads as the kinder answer to "come back later". |
| `PUT /api/account/payout-details` | Session user, non-staff | `UPDATE user … WHERE id = :session` — the route has no subject parameter at all. The response re-reads the row, so the gate reported is the one the caller's own save produced. |
| `DELETE /api/account/payout-details` | Session user | Refusal: `SELECT SUM(e.amount) FROM referral_earning e JOIN referral r … WHERE r.referrer_user_id = :session AND e.status='confirmed'` (a paid earning has flipped status, so `payout_id IS NULL` is implied). Then `UPDATE user SET payout_* = NULL WHERE id = :session`, batched with its `change` access row. |
| `GET /api/account/referrals` | Session user; **403 for `type='internal'`** | Per-query scoping as designed. See F3: an anonymous caller currently gets 200 with an empty gate rather than 401. |
| `POST /api/ops/referrals/payouts/mark-paid` | Staff | The body names `userIds` only; the server derives earnings from its own queue (`status='confirmed' AND payout_id IS NULL`). Both writes are guarded in one batch — the payout row inserts `WHERE EXISTS (… confirmed AND payout_id IS NULL)` and the claim updates the same predicate — so a concurrent double-submit cannot leave a payout row attached to no earnings. |
| `POST /api/ops/referrals/payouts/:id/failed` | Staff | `UPDATE referral_payout SET status='failed' WHERE id = :id` (the designed `AND status='paid'` guard is absent; re-running is a no-op because the earnings clause carries it) plus `UPDATE referral_earning … WHERE payout_id = :id AND status='paid'`. |
| `POST /api/ops/referrals/link` | Staff | No direct write: it delegates to `recordReferral`, which owns every gate. Subject resolved from `userId` or `email`; 404 when no such account. Cross-account **by role**, like every other ops route. |

**Default-deny re-verified.** No endpoint anywhere writes another account's payout details: `payout_bsb` / `payout_account_number` appear in exactly one module, written only by `savePayoutDetails` and `leaveProgram`, both keyed to the session user's own id. `unmaskedPayoutDetails` has exactly one caller (`payoutQueue`), and the CSV is built through the queue rather than from the `user` table — so the largest single disclosure in the feature cannot happen without its log row. **ADR-4 and the ops-edit prohibition hold as designed.**

### 18.3 Findings routed to the developer

| # | Design clause | Where | What |
|---|---|---|---|
| **F1** | §6.2 — "calls `recordReferralAtSignup` **inside a try/catch** (a referral failure must never fail a signup)" | `worker/routes/auth.ts:88` | The call is unguarded. Business refusals are returned rather than thrown, so the ordinary paths are safe — but a transient D1 error inside `recordReferral` throws out of `/verify` **after the account row was created and before the session is created**, failing the sign-in and losing the attribution permanently (the retry takes the existing-user branch, which by design never sees a code). Wrap in try/catch, log and continue. |
| **F2** | House rule "one place per fact"; §10.3 "review flags computed live per row" | `worker/lib/referral-discount.ts:118-126` and `worker/routes/ops-referrals.ts:157-167` | The three review-flag rules and their `same()` normaliser exist twice, verbatim — once in a lib module and once inline in a route handler. The postcode-flag removal had to be made in both; the next change will be made in one. Lift to a single exported function (e.g. `referralReviewFlags(referrer, referred)`) beside `opsReferralReview`, and call it from the list route. The label maps are duplicated the same way (`src/ops/ProjectRecord.tsx:29-34`, `src/ops/Referrals.tsx:231-233`) — one shared constant. |
| **F3** | §10.2 — `GET /api/account/referrals` is authed | `worker/routes/referrals.ts:31-36` | An unauthenticated caller gets **200** with `{referrerGate:{complete:false,…}, code:null}`, and the branch contains dead conditionals (`user ? … : null` inside the `!user` arm). Nothing leaks, but the contract says authed: return 401 and delete the dead expressions. |
| **F4** | §7.5 — "Off means *come back later*… there is no terminated state and no termination surface anywhere in this design — no ended notice" | `src/pages/ReferPage.tsx:71` and `src/components/referral/ReferrerBlock.tsx:157` render **"This program has ended."**; `src/ops/Referrals.tsx:200-202` tells the operator "Joining is paused — the landing page carries the come-back-later banner, the placements disappear" | One switch, two incompatible stories, and the customer-facing one uses the exact framing the owner removed when they cut the third state. The ops "What this says on the site" panel exists to show an operator the effect of the switch, and it currently misdescribes it. **One of the two must change**, and the ops panel must end up describing whatever the customer actually sees — see the decision recorded in §18.6. |

### 18.4 Design items not built (for the product-manager's acceptance pass)

Named here so that nothing is descoped silently. None is a structural defect; each is scope.

- **T9 entirely** — three of the four emails (`referral_recorded`, `referral_earning_confirmed`, `referral_discount_expiring`; only `referral_payout_sent` is wired), the rules/T&Cs content, and the `PrivacyPolicyPage` referral/payout section (spec §11.8).
- **§8 sweep limbs 1 and 2** — `referralSweep` implements only the late-confirmation limb. Expiry processing (`stripReferralFromDrafts` plus `expired_processed_at`) and the 30-day reminder (`reminder_sent_at`) are unimplemented; both columns exist, unused. Note that `EXPIRING_WITHIN_DAYS = 30` now lives client-side in `src/components/referral/format.ts` — when the reminder ships, that threshold must be one fact, not two.
- **`POST /api/ops/referrals/bulk-void`** (§10.3, AC-67) — the single void exists; the bulk action does not. §7.5 names the bulk void as the *only* way to stop in-flight promises, so today a pause can only be un-promised one row at a time.
- **`/api/ops/summary → referralPayoutsReady` and the ops dashboard "needs us" row** (§10.3, AC-46, UX §8.4).
- **The live badge on unissued surfaces** (§5.4, §10.2) — `GET /api/projects/current` and the price-preview response carry no `referral` field. The badge appears on the issued quote and the order (both from the issue stamp), and the discount is explained in the account section; AC-51/57's draft-side behaviour is the product-manager's call.

### 18.5 Re-verified and unchanged

- **`migrations/0051`** is additive only — `ALTER TABLE … ADD COLUMN`, `CREATE TABLE`, `CREATE INDEX`, one config `INSERT`. No DROP, no rebuild, no `UPDATE`, no cascade path touched (`d1-migration-safety` rule 1: no ceremony required; the export-first rule still applies at remote apply). It is the highest-numbered file. Deliberate and worth keeping: `referral_earning.order_id` and `referral_payout.referrer_user_id` carry **no** foreign key, so a future `"order"` or `user` rebuild cannot cascade-delete payout history — the exact class of incident this repo has already suffered.
- **ADR-1's acyclic graph** — `referral-discount.ts` imports `../types` and nothing else; `pricing.ts`, `orders.ts` and `referrals.ts` all read it as a leaf; `referrals.ts → lines.ts → pricing.ts → referral-discount.ts` terminates.
- **One place per fact** — one discount step in `computePrice`, exactly two callers of `loadAccountDiscount`, `previewSample` still with no user and no composition, `taxBreakdown` the only GST arithmetic (the earning's `base_amount` included), `issueQuote` still the freeze moment, no stored referral status anywhere, and `payoutComplete` never restated as SQL.
- **Ticket sequencing** — T0's golden corpus is a standalone commit predating every pricing change; the T3 re-ordering recorded in the build notes (payout details before `/r/`) was the right call and is adopted.
- **§13.1 suites** — as designed, plus a fourth: `scripts/tests/referral-codes.test.mjs` (pure code, ABN, CSV and email-vars cases, no server). `test:referral` runs all four and is in the `test` chain.

### 18.6 Held for the owner

**The Off-state copy (F4).** Recommendation: the paused framing — "Joining is paused while we rework the program — check back soon" — because it is what the owner decided when they cut the third state, it is what the ops screen already tells the operator, and "This program has ended" is a claim about the future that a returning program has to contradict. If the owner prefers the ended framing, §7.5 and this register are amended instead, and the ops restatement panel is corrected to match.

**A reason on payout reversal.** Not required by this design (§9 asks only that the failed row survive as history) and not built; required by UX §8.3. Recommendation: build it — `referral_payout.note` already exists, the ops void beside it already takes a reason, and the reason is what distinguishes "bounced, account closed" (the referrer's details are bad and next week's run will bounce again) from "recorded against the wrong row" (nothing to fix). If it is added, record the actor too: `reversePayout` writes no `paid_by` equivalent and no `logEvent`, so a reversal currently has no author.
