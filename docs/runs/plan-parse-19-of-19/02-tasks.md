...
1. [x] Add the reference label fixture and semantic gate without exposing fixture
   values to production code.
2. [x] RED/GREEN: enforce schedule-operation compatibility in `agentic_full` and
...
6. [x] Run focused suites after each slice, then the full gate with the recorded
   unrelated Sanity archive baseline exception.
7. [ ] Run the real reference set three times in `agentic_full`; promote only at
   19/19 with no regression in the legacy path.
