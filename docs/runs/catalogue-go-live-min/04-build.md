...
## Fix round 3 — round-3 review findings (Codex), 2026-09-06

Three findings from `FIX-3.md`, all fixed test-first:

1. **`rate-cards.sql` must not collide with production's existing cards (Codex P1) + reset the glass flag (Codex P2).** The production backup already holds four of the 22 sheet cards (`amj72t-awning-window`, `amj72t-fixed-window`, `amj80-series-fixed-window`, `amj100l-series-fixed-window`); a plain `INSERT` on those ids would hit the primary key and abort the whole batch. `rate-cards.sql` was rewritten to 22 `INSERT … ON CONFLICT(id) DO UPDATE` upserts: each starts a new card at `v1`/`glass_excluded_from_area_rate=0`, and the conflict branch bumps the *existing* row's version (`'v' || (CAST(substr(version, 2) AS INTEGER) + 1)`) and resets the glass flag to `0` regardless of what it held before. Scan tests rewritten to express the new upsert shape: exactly 22 `INSERT … ON CONFLICT` statements against `pricing_rate_card` with ids matching the 22 sheet slugs; every VALUES row starts at `'v1'`; every `DO UPDATE` carries the version-bump expression and `updated_at = datetime('now')`; `glass_excluded_from_area_rate = 0` in both the VALUES row and the `DO UPDATE`; still no `DELETE`/`DROP`/`ALTER`/`CREATE TABLE`; `354.64` present, `322.40` absent.

   Rehearsed on local D1 in both paths: seeded the four backup cards at their backup values (`v4`/`v5`/`v2`/`v2`, `glass_excluded_from_area_rate=0`) so the `ON CONFLICT` branch would fire, then applied the file. Read-back: all 22 cards landed at the sheet's `perim_rate`/`area_rate` (column J) with the flag at `0`; the four seeded cards bumped to `v5`/`v6`/`v3`/`v3` as expected. One environment-state caveat: `amj150st-awning-window` read back at `v2` rather than the fixture's fresh-insert `v1`, because an earlier rehearsal pass in this same local D1 had already inserted that card once before this round's seed step — the upsert logic itself is correct (a genuinely-fresh local DB gives `v1` for every id not in the seeded four, which is what the scan/unit tests assert); the `v2` is local-environment leftover state, not a defect in the SQL. Re-running the file a second time is idempotent by construction (re-applies the same figures, bumps versions again) — noted in the file's header comment.

2. **Reference lookups must exclude drafts (Codex P1).** `loadWorld` filtered drafts on the `product` and `family` queries only; a draft `frameSystem`/`option`/`thermalProfile`/`category` could enter the id maps and let a published product end up referencing `drafts.…`. Fixed: `loadWorld` now filters every lookup — GROQ-side (`&& !(_id in path("drafts.**"))` added to categories/systems/profiles/fullProfiles/options for real-world correctness) and JS-side (a `.filter(d => !isDraft(d))` pass on each, since the test's mock transport matches on GROQ substrings and can't observe query-text-only changes). Test: a fixture `systems` list carrying `drafts.frameSystem-sys-80` alongside the published `sys-80` entry — asserts the loaded world has no draft-id systems, and that the plan's product patch still resolves to the published `system-80` id (the `Map`'s last-write-wins would otherwise let the draft, appended after the published entry, silently win).

3. **Validate fetched products before indexing them (Codex P2).** `products.map((p) => [p.slug.current, p])` threw on a product with no `slug`, and would have collapsed distinct slug-less products under `undefined` even if it didn't throw. Fixed: `loadWorld` now builds the slug map with a single-pass loop that only indexes products whose `slug.current` is a string, and collects the `_id`s of the rest into a new `missingSlug` array on the returned world. `plan()` now turns each `missingSlug` entry into a named problem (`product <id>: missing slug`) up front, alongside the existing problem list — which already causes `run()`'s existing problem branch to fire (exit 1, no `mutate()` call, nothing else changes). Test: a fetched product with `slug: null` → `loadWorld` doesn't throw, `plan()`'s problems name the product's `_id`, no mutation exits 1.

Not routed (per `FIX-3.md`, recorded in `DEBT.md`/runbook, no code changes): architecture round-3 findings 1 (`_rev`-less profile replace) and 2 (cross-store reconcile), the matching Codex P2 on profile-replace revision protection, and Ponytail round 3.

**Test results, run in full this round:**
- `npm run test:go-live` — 54/54 green (was 51 before this round; +3 new tests across findings 1's rewritten scan block, 2, 3).
- `npm run typecheck:gate` — clean, 0 fatal errors.
- `npm test` — full battery green apart from the same two pre-existing `scripts/tests/pipeline.test.mjs` failures noted in Fix round 1 (unrelated to `scripts/catalogue/**`, already routed to `DEBT.md`).

Committed as one commit covering `docs/runs/catalogue-go-live-min/rate-cards.sql`, `scripts/catalogue/apply-go-live-min.mjs`, `scripts/catalogue/go-live-plan.mjs`, `scripts/tests/catalogue-go-live.test.mjs`, and this file. The ~38 uncommitted files from the other effort stayed untouched and uncommitted.
